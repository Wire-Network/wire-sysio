\chapter{常见问题}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2faq_8zh-cn}{}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2faq_8zh-cn}\index{常见问题@{常见问题}}
\doxysection*{一般问题}
...
Rapid\+JSON 是一个 C++ 库，用于解析及生成 JSON。读者可参考它的所有 \doxylink{features_8zh-cn_8md}{特点}。
...
Rapid\+JSON 借镜了 Rapid\+XML 的一些设计, 包括原位（\texorpdfstring{$\ast$}{*}in situ\texorpdfstring{$\ast$}{*}）解析、只有头文件的库。但两者的 API 是完全不同的。此外 Rapid\+JSON 也提供许多 Rapid\+XML 没有的特点。
\item Rapid\+JSON 是免费的么？

是的，它在 MIT 特許條款下免费。它可用于商业软件。详情请参看 \href{https://github.com/Tencent/rapidjson/blob/master/license.txt}{\texttt{ license.\+txt}}。
...
现时不支持。\+Rapid\+JSON 只支持严格的标准格式。宽松语法现时在这 \href{https://github.com/Tencent/rapidjson/issues/36}{\texttt{ issue}} 中进行讨论。
\end{DoxyEnumerate}

\doxysection*{DOM 与 SAX}
...
\item 什么是原位（\texorpdfstring{$\ast$}{*}in situ\texorpdfstring{$\ast$}{*}）解析？

原位解析会把 JSON 字符串直接解码至输入的 JSON 中。这是一个优化，可减少内存消耗及提升性能，但输入的 JSON 会被更改。进一步细节请参考 \doxylink{dom_8zh-cn_8md}{原位解析} 。
\item 什么时候会产生解析错误？

当输入的 JSON 包含非法语法，或不能表示一个值（如 Number 太大），或解析器的处理器中断解析过程，解析器都会产生一个错误。详情请参考 \doxylink{dom_8zh-cn_8md}{解析错误}。
\item 有什么错误信息？

错误信息存储在 {\ttfamily \doxylink{struct_parse_result}{Parse\+Result}}，它包含错误代号及偏移值（从 JSON 开始至错误处的字符数目）。可以把错误代号翻译为人类可读的错误讯息。
...
有两个 API 可用：含 allocator 的构造函数，以及 {\ttfamily Copy\+From()}。可参考 \doxylink{tutorial_8zh-cn_8md}{深复制 Value} 里的用例。
\item 为什么我需要提供字符串的长度？
...
\begin{DoxyEnumerate}
\item 为什么不仅仅用 {\ttfamily printf} 输出一个 JSON？为什么需要 {\ttfamily \doxylink{class_writer}{Writer}}？

最重要的是，{\ttfamily \doxylink{class_writer}{Writer}} 能确保输出的 JSON 是格式正确的。错误地调用 SAX 事件（如 {\ttfamily Start\+Object()} 错配 {\ttfamily End\+Array()}）会造成断言失败。此外，{\ttfamily \doxylink{class_writer}{Writer}} 会把字符串进行转义（如 {\ttfamily \textbackslash{}n}）。最后，{\ttfamily printf()} 的数值输出可能并不是一个合法的 JSON number，特别是某些 locale 会有数字分隔符。而且 {\ttfamily \doxylink{class_writer}{Writer}} 的数值字符串转换是使用非常快的算法来实现的，胜过 {\ttfamily printf()} 及 {\ttfamily iostream}。
...
\item 能否对所有非 \doxylink{struct_a_s_c_i_i}{ASCII} 字符输出成 {\ttfamily \textbackslash{}uxxxx} 形式？

可以。只要在 {\ttfamily \doxylink{class_writer}{Writer}} 中使用 {\ttfamily \doxylink{struct_a_s_c_i_i}{ASCII}\texorpdfstring{$<$}{<}\texorpdfstring{$>$}{>}} 作为输出编码参数，就可以强逼转义那些字符。
\end{DoxyEnumerate}
...
使用者可使用 {\ttfamily \doxylink{class_file_read_stream}{File\+Read\+Stream}} 去逐块读入文件。但若使用于原位解析，必须载入整个文件。
\item 我能否解析一个从网络上串流进来的 JSON？

可以。使用者可根据 {\ttfamily \doxylink{class_file_read_stream}{File\+Read\+Stream}} 的实现，去实现一个自定义的流。
\item 我不知道一些 JSON 将会使用哪种编码。怎样处理它们？

你可以使用 {\ttfamily \doxylink{class_auto_u_t_f_input_stream}{Auto\+UTFInput\+Stream}}，它能自动检测输入流的编码。然而，它会带来一些性能开销。
...
Rapid\+JSON 的 {\ttfamily \doxylink{class_encoded_input_stream}{Encoded\+Input\+Stream}} 可检测／跳过 BOM。{\ttfamily \doxylink{class_encoded_output_stream}{Encoded\+Output\+Stream}} 可选择是否写入 BOM。可参考 \doxylink{stream_8zh-cn_8md}{编码流} 中的例子。
\item 为什么会涉及大端／小端？
...
叶劲峰（\+Milo Yip，\href{https://github.com/miloyip}{\texttt{ miloyip}}）是 Rapid\+JSON 的原作者。全世界许多贡献者一直在改善 Rapid\+JSON。\+Philipp \doxylink{struct_a}{A}. Hartmann（\href{https://github.com/pah}{\texttt{ pah}}）实现了许多改进，也设置了自动化测试，而且还参与许多社区讨论。丁欧南（\+Don Ding，\href{https://github.com/thebusytypist}{\texttt{ thebusytypist}}）实现了迭代式解析器。\+Andrii Senkovych（\href{https://github.com/jollyroger}{\texttt{ jollyroger}}）完成了向 CMake 的迁移。\+Kosta（\href{https://github.com/Kosta-Github}{\texttt{ Kosta-\/\+Github}}）提供了一个非常灵巧的短字符串优化。也需要感谢其他献者及社区成员。
...
\item 为什么这个项目从 Google \doxylink{struct_code}{Code} 搬到 Git\+Hub？
