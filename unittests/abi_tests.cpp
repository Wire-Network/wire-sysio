...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> std::optional<abi_serializer> {
      return abi_serializer(sysio_contract_abi(abi), yield_fn());
   };
}
...
{ try {
   ...
   // ------------------------------------------------------------------------------------------
   BOOST_CHECK_EXCEPTION( abis.set_abi(sysio_contract_abi(abi), yield_fn()), invalid_type_inside_abi, log_exception );
   ...
   const char *my_other = R"=====(
    {
      ...
      "keyweight": {"key":"SYS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},
      "keyweight_arr": [{"key":"SYS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"100"},{"key":"SYS6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV", "weight":"200"}],
      "authority": {
         "threshold":"10",
...
{ try {
   ...
   abi_serializer abis(sysio_contract_abi(abi_def()), yield_fn());
...
{ try {
   ...
   BOOST_TEST("SYS65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == updauth.auth.keys[0].key.to_string({}));
   BOOST_TEST(57005u == updauth.auth.keys[0].weight);
   BOOST_TEST("SYS5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == updauth.auth.keys[1].key.to_string({}));
...
{ try {
   ...
   BOOST_TEST("SYS65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == newacct.owner.keys[0].key.to_string({}));
   BOOST_TEST(57005u == newacct.owner.keys[0].weight);
   BOOST_TEST("SYS5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == newacct.owner.keys[1].key.to_string({}));
   ...
   BOOST_TEST("SYS65rXebLhtk2aTTzP4e9x1AQZs7c5NNXJp89W8R3HyaA6Zyd4im" == newacct.active.keys[0].key.to_string({}));
   BOOST_TEST(57005u == newacct.active.keys[0].weight);
   BOOST_TEST("SYS5eVr9TVnqwnUBNwf9kwMTbrHvX5aPyyEG97dz2b2TNeqWRzbJf" == newacct.active.keys[1].key.to_string({}));
...
{ try {
   ...
   abi_serializer abis(sysio_contract_abi(v.as<abi_def>()), yield_fn());
...
BOOST_AUTO_TEST_CASE(version)
{
   try {
      ...
      BOOST_CHECK_THROW( abi_serializer(fc::json::from_string(R"({"version": "sysio::abi/9.0"})").as<abi_def>(), yield_fn()), unsupported_abi_version_exception );
      abi_serializer(fc::json::from_string(R"({"version": "sysio::abi/1.0"})").as<abi_def>(), yield_fn());
      abi_serializer(fc::json::from_string(R"({"version": "sysio::abi/1.1"})").as<abi_def>(), yield_fn());
   } FC_LOG_AND_RETHROW()
}
