...
BOOST_FIXTURE_TEST_CASE(payer_other_rejected_kv_set, kv_api_tester) {
   // Non-privileged contract cannot bill another account via kv_set
   create_accounts({"alice"_n});
   produce_block();
   BOOST_CHECK_THROW(run_action("tstpayeroth"_n), sysio::chain::table_access_violation);
}

BOOST_FIXTURE_TEST_CASE(payer_other_allowed_when_privileged_kv_set, kv_api_tester) {
   // Privileged sysio.* contract can bill another account via kv_set
   // without explicit payer authorization (system contract exemption)
   name sys_test = "sysio.kvtest"_n;
   create_accounts({sys_test, "alice"_n});
   produce_block();
   set_code(sys_test, test_contracts::test_kv_api_wasm());
   set_abi(sys_test, test_contracts::test_kv_api_abi().c_str());
   set_privileged(sys_test);
   produce_block();

   signed_transaction trx;
   trx.actions.emplace_back(
      vector<permission_level>{{sys_test, config::active_name}},
      sys_test, "tstpayerprv"_n, bytes{}
   );
   set_transaction_headers(trx);
   trx.sign(get_private_key(sys_test, "active"), control->get_chain_id());
   BOOST_CHECK_NO_THROW(push_transaction(trx));
}
