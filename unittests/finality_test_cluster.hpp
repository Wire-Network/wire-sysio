...
// ----------------------------------------------------------------------------
struct finality_node_t : public sysio::testing::tester {
   using vote_message_ptr = sysio::chain::vote_message_ptr;
   using vote_result_t    = sysio::chain::vote_result_t;
   ...
   sysio::chain::vote_message_ptr          orig_vote;
...
public:
   ...
   finality_test_cluster(finality_cluster_config_t config = {.transition_to_savanna = true}) {
      ...
      // -----------------------------------------------------------------------------------------
      node0.control->aggregated_vote().connect( [&]( const sysio::chain::vote_signal_params& v ) {
   ...
   sysio::testing::produce_block_result_t produce_and_push_block_ex() {
...
public:
   ...
   std::optional<sysio::chain::finalizer_policy> fin_policy_0;         // policy used to transition to Savanna
...
private:
   ...
   void setup_node(finality_node_t& node, sysio::chain::account_name local_finalizer);
   ...
   vote_result_t process_vote(vote_message_ptr& vote, bool duplicate) {
      ...
      if (sysio::chain::block_header::num_from_id(vote->block_id) > node0.lib_num())
