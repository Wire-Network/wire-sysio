...
BOOST_AUTO_TEST_CASE_TEMPLATE( basic_test, T, validating_testers ) try {
   ...
   transaction_id_type yes_assert_id;
   {
      ...
      BOOST_CHECK_THROW(chain.push_transaction( trx ), sysio_assert_message_exception);
   }
...
BOOST_AUTO_TEST_CASE_TEMPLATE( abi_from_variant, T, validating_testers ) try {
   ...
   auto resolver = [&]( const account_name& name ) -> std::optional<abi_serializer> {
      try {
         if (const auto* accnt  = chain.control->find_account_metadata( name ); accnt != nullptr) {
...
BOOST_AUTO_TEST_CASE_TEMPLATE( table_init_oob, T, validating_testers ) try {
   ...
   trx.actions[0].authorization = vector<permission_level>{{"tableinitoob"_n,config::active_name}};
    ...
    auto pushit_and_expect_fail = [&]() {
      ...
      BOOST_CHECK_THROW(chain.push_transaction(trx), sysio::chain::wasm_exception);
   };
   ...
   BOOST_CHECK_THROW(chain.set_code("tableinitoob"_n, table_init_oob_no_table_wast), sysio::chain::wasm_exception);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( memory_init_border, T, validating_testers ) try {
   ...
   BOOST_CHECK_THROW(chain.set_code("memoryborder"_n, memory_init_toolong), sysio::chain::wasm_exception);
   BOOST_CHECK_THROW(chain.set_code("memoryborder"_n, memory_init_negative), sysio::chain::wasm_exception);
...
BOOST_AUTO_TEST_CASE_TEMPLATE( nested_limit_test, T, validating_testers ) try {
   ...
   }
   {
      ...
      for(unsigned int i = 0; i < 1024; ++i)
         ss << ")";
      ss << "))";
      BOOST_CHECK_THROW(chain.set_code("nested"_n, ss.str().c_str()), sysio::chain::wasm_exception);
   }
...
BOOST_AUTO_TEST_CASE_TEMPLATE( noop, T, validating_testers ) try {
   ...
   const auto* accnt  = chain.control->find_account_metadata("noop"_n);
   abi_def abi;
...
// abi_serializer::to_variant failed because sysio_system_abi modified via chain.set_abi.
...
BOOST_AUTO_TEST_CASE_TEMPLATE( sysio_abi, T, validating_testers ) try {
   T chain;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( check_table_maximum, T, validating_testers ) try {
   ...
   BOOST_CHECK_THROW(chain.push_transaction(trx), sysio::chain::wasm_execution_error);
   }
   ...
   BOOST_CHECK_EXCEPTION(chain.push_transaction(trx), sysio::chain::wasm_execution_error, [](const sysio::chain::wasm_execution_error &e) {return true;});
   }
} FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE_TEMPLATE( billed_cpu_test, T, testers ) try {
   ...
   auto r = push_trx( ptrx, fc::time_point::maximum(), cpu_limit-1, false );
   ...
   fc::microseconds subjective_cpu_bill_us = leeway;
   chain.control->get_mutable_subjective_billing().reset();
   chain.control->get_mutable_subjective_billing().subjective_bill_failure({}, {{acc, subjective_cpu_bill_us}}, fc::time_point::now());
   uint32_t billed_cpu_time_us = SYS_PERCENT( (combined_cpu_limit - subjective_cpu_bill_us.count()), 89 *config::percent_1 );
   push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false );
   ...
   chain.control->get_mutable_subjective_billing().reset();
   subjective_cpu_bill_us = fc::microseconds{0};
   billed_cpu_time_us = SYS_PERCENT( combined_cpu_limit - subjective_cpu_bill_us.count(), 89 *config::percent_1 );
   push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false );
   ...
   subjective_cpu_bill_us = fc::microseconds{cpu_limit};
   chain.control->get_mutable_subjective_billing().reset();
   chain.control->get_mutable_subjective_billing().subjective_bill_failure({}, {{acc, subjective_cpu_bill_us}}, fc::time_point::now());
   billed_cpu_time_us = SYS_PERCENT( combined_cpu_limit - subjective_cpu_bill_us.count(), 90 * config::percent_1 );
   ...
   subjective_cpu_bill_us = fc::microseconds{0};
   chain.control->get_mutable_subjective_billing().reset();
   billed_cpu_time_us = SYS_PERCENT( combined_cpu_limit - subjective_cpu_bill_us.count(), 91 * config::percent_1 );
   ptrx->prev_accounts_billing = accounts_billing_t{{acc, {.cpu_usage_us = billed_cpu_time_us}}};
   BOOST_CHECK_EXCEPTION(push_trx( ptrx, fc::time_point::maximum(), billed_cpu_time_us, false ), tx_cpu_usage_exceeded,
                         [](const tx_cpu_usage_exceeded& e){ fc_exception_message_starts_with starts("estimated");
                                                             fc_exception_message_contains contains_reached("reached account cpu limit");
...
BOOST_AUTO_TEST_CASE( more_billed_cpu_test ) try {
   ...
   // indicate explicit billing at transaction minimum
   cpu_usage = {25, 25, 50};
   push_trx( ptrx, fc::time_point::maximum(), cpu_usage, true );
   chain.produce_block();
   ...
   cpu_usage = {25, 25, 49};
   // indicate explicit billing at minimum-1, objective failure even with explicit billing for under min
   BOOST_CHECK_EXCEPTION( push_trx( ptrx, fc::time_point::maximum(), cpu_usage, true ), transaction_exception,
                          fc_exception_message_is("cannot bill CPU time 99 less than the minimum of 100 us") );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( sysio_exit_in_start, T, validating_testers ) try {
   T chain;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( memory_mapping_test, T, validating_testers ) try {
   ...
   for (uint32_t i = 1; i < num_contracts; ++i) {
      ...
      chain.create_accounts({acct}, false, true, false);
      chain.add_roa_policy(chain.NODE_DADDY, acct, "0.0001 SYS", "0.0001 SYS", "0.0200 SYS", 0, 0);
      ...
      chain.push_action(acct, "dothedew"_n, "sysio"_n, {});
