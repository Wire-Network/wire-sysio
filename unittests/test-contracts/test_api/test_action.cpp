...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( code == receiver ) {
      ...
      if( kv_contains( receiver, key, 24 ) )
         kv_erase( key, 24 );

      // Create the row simply for the purpose of charging code more RAM.
      if( payer != 0 )
         kv_set( 0, payer, key, 24, &to_notify, sizeof(to_notify) );
   }
}
