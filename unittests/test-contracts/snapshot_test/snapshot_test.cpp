...
void snapshot_test::add(sysio::name scope, uint64_t id, sysio::checksum256 payload) {
   require_auth(get_self());
...
void snapshot_test::remove(sysio::name scope, uint64_t id) {
   require_auth(get_self());
...
void snapshot_test::verify(sysio::name scope, uint64_t id, sysio::checksum256 payload) {
   require_auth(get_self());
