...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, validating_tester) { try {
      ...
      BOOST_CHECK_EXCEPTION(push_transaction(trx), transaction_exception, fc_exception_message_is("Context free data size 2 not equal to context free actions size 1"));
...
void push_trx(Tester& test, T ac, uint32_t billed_cpu_time_us , uint32_t max_cpu_usage_ms, uint32_t max_block_cpu_ms,
   ...
   auto res = test.control->test_push_transaction( trx_meta, fc::time_point::now() + fc::milliseconds(max_block_cpu_ms),
                                              fc::milliseconds(max_cpu_usage_ms), billed_cpu_us, explicit_bill, 0 );
...
void transaction_tests(T& chain) {
   ...
   // test read_transaction only returns packed transaction
   {
      ...
      auto r = chain.control->test_push_transaction( fut.get(), fc::time_point::maximum(), fc::microseconds::maximum(), billed_cpu_us, true, 0 );
