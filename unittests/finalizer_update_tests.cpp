...
BOOST_AUTO_TEST_CASE(savanna_set_finalizer_single_test) { try {
   ...
   auto finpol = t.active_finalizer_policy(t.head().id());
   ...
   t.produce_block();
   BOOST_TEST(finpol->generation == 2u);
   ...
   t.check_head_finalizer_policy(3u, pubkeys1); // two 2-chain - new policy *should* be active
...
BOOST_AUTO_TEST_CASE(savanna_set_finalizer_multiple_test) { try {
   ...
   BOOST_TEST(finpol->generation == 2u);
   t.produce_block();
   t.check_head_finalizer_policy(3u, pubkeys2); // two 2-chain - new policy pubkeys2 *should* be active
   ...
   verify_block_finality_policy_diff(b, 4, pubkeys3.back());
   auto pubkeys4 = fin_keys.set_finalizer_policy(4u).pubkeys;
   b = t.produce_block(); // proposed: pubkeys3, pubkeys4
   verify_block_finality_policy_diff(b, 5, pubkeys4.back());
   ...
   verify_block_finality_policy_diff(b, 6, pubkeys5.back());
   t.check_head_finalizer_policy(3u, pubkeys2); // 3 blocks after pubkeys3 proposed, pubkeys2 should still be active
   t.produce_block();  // pubkeys3 becomes active after 4 blocks -- active: pubkeys3, pending: pubkeys4, proposed: pubkeys5
   t.check_head_finalizer_policy(4u, pubkeys3);
   auto pubkeys6 = fin_keys.set_finalizer_policy(6u).pubkeys;
   b = t.produce_block(); // pending: pubkeys4, proposed: pubkeys5, pubkeysr6
   verify_block_finality_policy_diff(b, 7, pubkeys6.back());
   ...
   verify_block_finality_policy_diff(b, 8, pubkeys7.back());
   ...
   t.check_head_finalizer_policy(5u, pubkeys4);
   verify_block_finality_policy_diff(b, 9, pubkeys8.back());
   ...
   verify_block_finality_policy_diff(b, 10, pubkeys9.back());
   auto pubkeys10 = fin_keys.set_finalizer_policy(10u).pubkeys;
   t.check_head_finalizer_policy(6u, pubkeys5);
   b = t.produce_block(); // active: pubkeys5, pending: pubkeys7, proposed: pubkeys8, pubkeys9 -- pubkeysr6 garbage collected
   verify_block_finality_policy_diff(b, 11, pubkeys10.back());
   auto pubkeys11 = fin_keys.set_finalizer_policy(11u).pubkeys;
   t.check_head_finalizer_policy(6u, pubkeys5);
   b = t.produce_block();
   verify_block_finality_policy_diff(b, 12, pubkeys11.back());
   t.produce_block();
   t.check_head_finalizer_policy(8u, pubkeys7); // the rest are all one block apart, tests pending with propsed
   auto b12 = t.produce_block();
   t.check_head_finalizer_policy(10u, pubkeys9);
   auto b13 = t.produce_block();
   t.check_head_finalizer_policy(10u, pubkeys9);
   auto b14 = t.produce_block();
   t.check_head_finalizer_policy(12u, pubkeys11);

   // and no further change
   // ---------------------
   for (size_t i=0; i<10; ++i)
      ensure_next_block_finalizer_policy(t, 12u, pubkeys11);
} FC_LOG_AND_RETHROW() }
