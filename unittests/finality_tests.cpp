...
BOOST_AUTO_TEST_CASE(threshold_equal_to_half_weight_sum_test) { try {
   ...
   BOOST_REQUIRE_THROW( test_finality_transition(account_names, policy_input, false), sysio_assert_message_exception );
...
BOOST_FIXTURE_TEST_CASE(unknown_finalizer_key_votes, finality_test_cluster<4>) { try {
   ...
   process_vote(1, 0);
   BOOST_REQUIRE(process_vote(1, 0) == sysio::chain::vote_result_t::unknown_public_key);
