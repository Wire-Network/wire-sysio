...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.set_code("sysio.token"_n, contracts::sysio_token_wasm());
   chain.set_abi("sysio.token"_n, contracts::sysio_token_abi().data());
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>("sysio.token"_n);
   ...
   const auto a = chain.control->db().get<account_object,by_name>("sysio.token"_n).get_abi();
