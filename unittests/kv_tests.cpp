...
BOOST_AUTO_TEST_CASE(kv_index_modify_rekeys_correctly) {
   ...
   // Create three primary rows that the secondary entries will reference.
   const auto& alice_row = db.create<kv_object>([&](auto& o) {
      o.code = "test"_n;
      o.table_id = users_tid;
      o.key.assign("\x00\x01", 2);
   ...
   const auto& charlie_row = db.create<kv_object>([&](auto& o) {
      o.code = "test"_n;
      o.table_id = users_tid;
      o.key.assign("\x00\x03", 2);
      o.value.assign("charlie_val", 11);
   });
   ...
   // primary_id is preserved across the sec_key rekey.
   BOOST_CHECK(itr->primary_id == alice_row.id);

   // Lookup by full composite key (sec_key + primary_id) finds the modified object.
