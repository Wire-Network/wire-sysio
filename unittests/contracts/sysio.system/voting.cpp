...
void system_contract::regproducer( const name producer, const sysio::public_key& producer_key, const std::string& url, uint16_t location ) {
   ...
   check( producer_key != sysio::public_key(), "public key should not be the default value" );
...
void system_contract::update_elected_producers( block_timestamp block_time ) {
   ...
   std::vector< std::pair<sysio::producer_key,uint16_t> > top_producers;
   top_producers.reserve(21);

   for ( auto it = idx.cbegin(); it != idx.cend() && top_producers.size() < 21 && 0 < it->total_votes && it->active(); ++it ) {
      top_producers.emplace_back( std::pair<sysio::producer_key,uint16_t>({{it->owner, it->producer_key}, it->location}) );
   }
   ...
   std::vector<sysio::producer_key> producers;
