...
void system_contract::deposit( const name& owner, const asset& amount )
{
   ...
   INLINE_ACTION_SENDER(sysio::token, transfer)( token_account, { owner, active_permission },
...
void system_contract::withdraw( const name& owner, const asset& amount )
{
   ...
   INLINE_ACTION_SENDER(sysio::token, transfer)( token_account, { rex_account, active_permission },
...
void system_contract::unstaketorex( const name& owner, const name& receiver, const asset& from_net, const asset& from_cpu )
{
   ...
   INLINE_ACTION_SENDER(sysio::token, transfer)( token_account, { stake_account, active_permission },
...
void system_contract::channel_to_rex( const name& from, const asset& amount )
{
   ...
   if ( rex_available() ) {
         ...
         INLINE_ACTION_SENDER(sysio::token, transfer)( token_account, { from, active_permission },
            { from, rex_account, amount, std::string("transfer from ") + name{from}.to_string() + " to sysio.rex"} );
      }
#endif
}
