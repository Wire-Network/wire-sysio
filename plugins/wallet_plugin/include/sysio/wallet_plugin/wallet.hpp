...
public:
   explicit soft_wallet(const wallet_data& initial_data);
   ...
   /**
    *
    * @param key_name alias or name
    * @return Private key
    */
   private_key_type get_private_key(string key_name) const override;
   ...
   map<string, wallet_key_entry> list_keys_by_name() override;
   ...
   /**
    * Import a key and assign a specific name
    *
    * @param key_name string name mapped to key
    * @param wif_key string WIF private key
    * @return true if successful
    */
   bool import_key(string key_name, string wif_key) override;
    ...
    */
   bool remove_key(public_key_type key) override;
   ...
   */
   ...
   string create_key(string key_name, string key_type) override;

   void set_key_name(string current_key_name, string new_key_name) override;
   void set_key_name(private_key_type private_key, string key_name) override;
   void set_key_name(public_key_type public_key, string key_name) override;

   string generate_key_name(string prefix = "", string suffix = "") override;
...
struct plain_keys_v1 {
   fc::sha512                             checksum;
   map<public_key_type, private_key_type> keys;
   map<string, public_key_type>           key_by_name;
};
