...
struct yubihsm_wallet_impl {
   ...
   public_key_type create() {
      ...
      try {
         if((rc = yh_util_generate_ec_key(session, &new_key_id, "kiod created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
...
string yubihsm_wallet::create_key(string key_type) {
   SYS_ASSERT(key_type.empty() || key_type == "R1", chain::unsupported_key_type_exception, "YubiHSM wallet only supports R1 keys");
