...
struct session_base {
   ...
   virtual void send_update(const sysio::chain::block_state_legacy_ptr& block_state) = 0;
...
class blocks_ack_request_send_queue_entry : public send_queue_entry_base {
   std::shared_ptr<Session> session;
   sysio::state_history::get_blocks_ack_request_v0 req;
...
class blocks_request_send_queue_entry : public send_queue_entry_base {
   std::shared_ptr<Session> session;
   sysio::state_history::get_blocks_request_v0 req;
...
private:
   ...
   uint64_t get_trace_log_entry(const sysio::state_history::get_blocks_result_v0& result,
   ...
   uint64_t get_delta_log_entry(const sysio::state_history::get_blocks_result_v0& result,
                                std::optional<locked_decompress_stream>& buf) {
   ...
   void send_update(state_history::get_blocks_result_v0 result, const chain::block_state_legacy_ptr& block_state) {
      ...
      if (block_id && position_it && (*position_it)->block_num == to_send_block_num) {
         // This branch happens when the head block of nodeop is behind the head block of connecting client.
