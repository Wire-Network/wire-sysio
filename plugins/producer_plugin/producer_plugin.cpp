...
public:
   ...
   producer_plugin::snapshot_information finalize( const chain::controller& chain ) const {
      ...
      if (!in_chain) {
         ...
         SYS_THROW(snapshot_finalization_exception,
...
public:
   ...
   explicit account_failures( const sysio::subjective_billing& sb )
...
private:
   ...
   const sysio::subjective_billing& subjective_billing;
};
   ...
   boost::program_options::options_description& config_file_options)
{
   ...
   producer_options.add_options()
         ...
         ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value(
          ...
          "   KSYSD:<data>    \tis the URL where kiod is available and the approptiate wallet(s) are unlocked")
         ("kiod-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowed for sending blocks to a kiod provider for signing")
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    SYS_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_plugin_impl::signature_provider_type
make_kiod_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kiod_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kiod_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), std::optional<uint16_t>());
   else
      kiod_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kiod_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kiod_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kiod_provider_timeout_us : fc::time_point::maximum();
...
{ try {
   ...
   SYS_ASSERT( my->chain_plug, plugin_config_exception, "chain_plugin not found" );
   my->_options = &options;
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            SYS_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            SYS_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
            ...
            } else if (spec_type_str == "KSYSD") {
               my->_signature_providers[pubkey] = make_kiod_signature_provider(my, spec_data, pubkey);
            }
   ...
   SYS_ASSERT( my->_produce_time_offset_us <= 0 && my->_produce_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   SYS_ASSERT( my->_last_block_time_offset_us <= 0 && my->_last_block_time_offset_us >= -config::block_interval_us, plugin_config_exception,
   ...
   SYS_ASSERT( cpu_effort_pct >= 0 && cpu_effort_pct <= 100, plugin_config_exception,
   ...
   SYS_ASSERT( last_block_cpu_effort_pct >= 0 && last_block_cpu_effort_pct <= 100, plugin_config_exception,
   ...
   SYS_ASSERT( my->_max_block_cpu_usage_threshold_us < config::block_interval_us, plugin_config_exception,
   ...
   SYS_ASSERT( subjective_account_decay_time.count() > 0, plugin_config_exception,
   ...
   SYS_ASSERT( thread_pool_size > 0, plugin_config_exception,
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      SYS_ASSERT( fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   ...
   chain::controller& chain = my->chain_plug->chain();
   SYS_ASSERT( my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
              "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"" );

   SYS_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
              "node cannot have any producer-name configured because block production is not safe when validation_mode is not \"full\"" );

   SYS_ASSERT( my->_producers.empty() || my->chain_plug->accept_transactions(), plugin_config_exception,
...
void producer_plugin::set_whitelist_blacklist(const producer_plugin::whitelist_blacklist& params) {
   SYS_ASSERT(params.actor_whitelist || params.actor_blacklist || params.contract_whitelist || params.contract_blacklist || params.action_blacklist || params.key_blacklist,
...
void producer_plugin::schedule_protocol_feature_activations( const scheduled_protocol_feature_activations& schedule ) {
   ...
   SYS_ASSERT( set_of_features_to_activate.size() == schedule.protocol_features_to_activate.size(),
