...
chain::transaction_trace_ptr make_transaction_trace( const packed_transaction_ptr trx, uint32_t block_number, const sysio::chain::block_state_legacy_ptr& bs_ptr ) {
...
BOOST_AUTO_TEST_CASE(trx_finality_status_logic) { try {
   ...
   uint32_t bn = 20;
   auto add = [&bn, &status](trx_deque& trx_pairs, const sysio::chain::block_state_legacy_ptr& bs_ptr) {
   ...
   const sysio::chain::block_state_legacy_ptr no_bs;
