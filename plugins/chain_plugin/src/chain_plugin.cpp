...
/// Poll interval while an off-thread caller waits for the app executor or a
/// transaction callback. Short enough to observe shutdown promptly without a
/// busy wait.
...
signed_action_detail::create_signed_action_submission_callback(
   ...
   return [read_write_api = std::move(read_write_api),
      ...
      const auto& processed = std::get<
         chain_apis::read_write::push_transaction_results>(result).processed;
      if (processed.is_object() && processed.get_object().contains("except") &&
          !processed["except"].is_null()) {
         promise->set_value({
            .state = signed_action_result::status::rejected,
            .error = "transaction execution reported an exception",
         });
         return;
      }
...
chain_plugin::read_table_rows(chain_apis::read_only::get_table_rows_params params,
   ...
   while (fut.wait_for(executor_wait_poll_interval) == std::future_status::timeout) {
...
chain_plugin::push_signed_action(chain::name contract,
   ...
   const auto wait_until_ready = [&](auto& future) -> bool {
   ...
   try {
      ...
      auto future = promise->get_future();
      auto submit = [this, params = packed_variant.get_object(), promise,
                     timeout]() mutable {
         try {
            ...
            promise->set_value({.state = status::rejected,
                                .error = e.to_detail_string()});
         } catch (const std::exception& e) {
            promise->set_value({.state = status::rejected, .error = e.what()});
         } catch (...) {
            promise->set_value({.state = status::rejected,
                                .error = "unknown transaction submission failure"});
         }
      };

      // `push_transaction` resolves ABI data before it queues the incoming
      // transaction. Run that work in the controller read window too, rather
      // than letting a cron worker touch controller state directly. The appbase
      // executor is drained before plugin destruction, so its queued lambda may
      // safely retain `this` if this caller times out first.
      if (std::this_thread::get_id() == app().executor().get_main_thread_id()) {
         submit();
      } else {
         app().executor().post(
