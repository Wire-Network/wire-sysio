 ...
 * @brief A class that handles HTTP requests to an Solana node using libcurl.
 *
 * This class provides methods to send HTTP POST requests to an Solana node,
 * retrieve the response, and handle the low-level network communication using libcurl.
 * It is designed to be used by other classes to interact with an Solana node.
 */
...
public:
    ...
    * @param url The URL to send the POST request to (e.g., the Solana node endpoint).
