...
#include <sysio/outpost_client/solana/network_adapter.hpp>
...
/**
 * @class solana_client
 * @brief A class to interact with an Solana or Solana-compatible node.
 *
 * This class provides methods to send RPC requests to an Solana node,
 * parse the responses, and process the results. It supports common Solana RPC methods
 * such as retrieving block data, transaction details, estimating gas, and more.
 */
class LIB_EXPORT solana_client {
public:
    /**
     * @brief Constructs an SolanaClient instance.
     * @param node_url The URL of the Solana node (e.g., Infura, local node).
     * @param net_adapter A reference to the NetworkAdapter used for sending requests.
     */
    solana_client(const std::string& node_url, network_adapter& net_adapter);
    ...
    /**
     * @brief Parses the response from the Solana node.
     * @param response The raw response from the Solana node.
...
LIB_EXPORT
    /**
     * @brief Retrieves the chain ID of the connected Solana network.
     * @return The chain ID as a string, or an empty std::optional if an error occurs.
     */
    std::optional<std::string> get_chain_id();

    /**
     * @brief Retrieves the version of the connected Solana network.
    ...
    /**
     * @brief Checks if the Solana node is syncing.
 ...
 private:
    std::string      node_url_;         ///< The URL of the Solana node.
