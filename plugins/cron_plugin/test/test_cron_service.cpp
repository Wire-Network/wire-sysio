...
BOOST_AUTO_TEST_SUITE(cron_service)
  ...
  BOOST_AUTO_TEST_CASE(create_with_options_and_basic_add) try {
    ...
    svc::job_schedule s;
    s.milliseconds.insert(svc::job_schedule::step_value{1000}); // every second
  ...
  BOOST_AUTO_TEST_CASE(milliseconds_field_precise_triggers_same_second) try {
    ...
    svc::job_schedule s;
    s.milliseconds.insert(svc::job_schedule::exact_value{static_cast<std::uint64_t>((now_ms + 100) % 60000)});
    s.milliseconds.insert(svc::job_schedule::exact_value{static_cast<std::uint64_t>((now_ms + 200) % 60000)});
    s.milliseconds.insert(svc::job_schedule::exact_value{static_cast<std::uint64_t>((now_ms + 300) % 60000)});
  ...
  BOOST_AUTO_TEST_CASE(multiple_independent_jobs_progress) try {
    ...
    svc::job_schedule fast;
    fast.milliseconds.insert(svc::job_schedule::step_value{1000}); // every second
  ...
  BOOST_AUTO_TEST_CASE(test_expand_field_exact) try {
    std::set<svc::job_schedule::schedule_value> field;
    field.insert(svc::job_schedule::exact_value{5});
    field.insert(svc::job_schedule::exact_value{10});
    field.insert(svc::job_schedule::exact_value{15});

    auto result = svc::job_schedule::expand_field(field, 0, 59);
  ...
  BOOST_AUTO_TEST_CASE(test_expand_field_step) try {
    std::set<svc::job_schedule::schedule_value> field;
    field.insert(svc::job_schedule::step_value{15});
  ...
  BOOST_AUTO_TEST_CASE(test_expand_field_range) try {
    std::set<svc::job_schedule::schedule_value> field;
    field.insert(svc::job_schedule::range_value{3, 7});

    auto result = svc::job_schedule::expand_field(field, 1, 31);
  ...
  BOOST_AUTO_TEST_CASE(test_expand_field_wildcard) try {
    std::set<svc::job_schedule::schedule_value> field; // empty = wildcard
    auto result = svc::job_schedule::expand_field(field, 0, 59);
  ...
  BOOST_AUTO_TEST_CASE(test_step_value_schedule) try {
    ...
    svc::job_schedule s;
    s.milliseconds.insert(svc::job_schedule::step_value{250});
  ...
  BOOST_AUTO_TEST_CASE(test_range_value_schedule) try {
    ...
    svc::job_schedule s;
    s.minutes.insert(svc::job_schedule::range_value{0, 2});
    s.milliseconds.insert(svc::job_schedule::exact_value{0});
  ...
  BOOST_AUTO_TEST_CASE(test_next_fire_time_algorithm) try {
    ...
    svc::job_schedule s;
    s.hours.insert(svc::job_schedule::exact_value{14});
    s.minutes.insert(svc::job_schedule::exact_value{30});
    s.milliseconds.insert(svc::job_schedule::exact_value{0});
  ...
  BOOST_AUTO_TEST_CASE(test_trigger_precomputation) try {
    ...
    svc::job_schedule s;
    s.minutes.insert(svc::job_schedule::exact_value{0});
    s.milliseconds.insert(svc::job_schedule::exact_value{0});
  ...
  BOOST_AUTO_TEST_CASE(test_day_of_week_schedule) try {
    ...
    svc::job_schedule s;
    s.day_of_week.insert(svc::job_schedule::exact_value{3}); // Wednesday
    s.hours.insert(svc::job_schedule::exact_value{12});
    s.minutes.insert(svc::job_schedule::exact_value{0});
    s.milliseconds.insert(svc::job_schedule::exact_value{0});
  ...
  // -----------------------------------------------------------------------
  // blocking_retry tests
  // -----------------------------------------------------------------------

  namespace {
    svc::retry_options fast_retry_opts(int max_retries = 5) {
  ...
  BOOST_AUTO_TEST_CASE(blocking_retry_succeeds_immediately) try {
    auto service = cron_service_factory("blocking_retry_immediate", 2);
    service->start();

    int calls = 0;
    auto fn = [&]() -> std::optional<int> {
      ++calls;
      return 42;
    };
    auto result = service->blocking_retry(fast_retry_opts(), fn);

    BOOST_REQUIRE(result.has_value());
    BOOST_CHECK_EQUAL(*result, 42);
    BOOST_CHECK_EQUAL(calls, 1); // first-try success, no retries
  } FC_LOG_AND_RETHROW();

  BOOST_AUTO_TEST_CASE(blocking_retry_retries_until_success) try {
    auto service = cron_service_factory("blocking_retry_eventually", 2);
    service->start();

    std::atomic_int calls{0};
    auto fn = [&]() -> std::optional<int> {
      if (calls.fetch_add(1) < 2) return std::nullopt; // fail first 2 attempts
      return 7;
    };
    auto result = service->blocking_retry(fast_retry_opts(10), fn);

    BOOST_REQUIRE(result.has_value());
    BOOST_CHECK_EQUAL(*result, 7);
    BOOST_CHECK_GE(calls.load(), 3);
  } FC_LOG_AND_RETHROW();

  BOOST_AUTO_TEST_CASE(blocking_retry_exhausts_budget) try {
    auto service = cron_service_factory("blocking_retry_exhaust", 2);
    service->start();
    ...
    auto result = service->blocking_retry(fast_retry_opts(3), fn);

    BOOST_CHECK(!result.has_value());
    // initial call + up to max_retries attempts
    BOOST_CHECK_GE(calls.load(), 2);
  } FC_LOG_AND_RETHROW();

  BOOST_AUTO_TEST_CASE(blocking_retry_propagates_throw) try {
    auto service = cron_service_factory("blocking_retry_throw", 2);
    service->start();

    // Initial call returns empty so retries engage; retry throws.
    std::atomic_int calls{0};
    auto fn = [&]() -> std::optional<int> {
      const int n = calls.fetch_add(1);
      if (n == 0) return std::nullopt;
      FC_THROW_EXCEPTION(fc::assert_exception, "test retry failure");
    };
    auto result = service->blocking_retry(fast_retry_opts(), fn);
  ...
  BOOST_AUTO_TEST_CASE(blocking_retry_requires_multiple_threads) try {
    auto service = cron_service_factory("blocking_retry_single_thread", 1);
    service->start();

    auto fn = []() -> std::optional<int> { return std::nullopt; };
    BOOST_CHECK_THROW(service->blocking_retry(fast_retry_opts(), fn), fc::exception);
  } FC_LOG_AND_RETHROW();
