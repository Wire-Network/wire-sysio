...
void net_api_plugin::plugin_startup() {
   ...
   app().get_plugin<http_plugin>().add_async_api_stream({
       CALL_WITH_400_STREAM(net, net_rw, net_mgr, connect,
            INVOKE_R_R(net_mgr, connect, std::string), 201),
       CALL_WITH_400_STREAM(net, net_rw, net_mgr, disconnect,
            INVOKE_R_R(net_mgr, disconnect, std::string), 201),
       CALL_WITH_400_STREAM(net, net_ro, net_mgr, status,
            INVOKE_R_R(net_mgr, status, std::string), 201),
       CALL_WITH_400_STREAM(net, net_ro, net_mgr, connections,
            INVOKE_R_V(net_mgr, connections), 201),
       CALL_WITH_400_STREAM(net, net_ro, net_mgr, bp_gossip_peers,
