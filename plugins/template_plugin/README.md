...
`template_plugin` is the scaffold a new `nodeop` plugin starts from: the smallest complete appbase plugin
that still has the right shape — a `plugin_target` CMake file, a public header declaring the four lifecycle
methods, and a source file with a private `impl` struct behind a `unique_ptr`. It implements no behavior,
registers no options, and is deliberately absent from `plugins/CMakeLists.txt`, so it is never compiled into
`nodeop` and there is nothing for an operator to enable. Copy it (or generate a copy with
`npx plop create-cxx-plugin`) when adding a plugin.

## What it demonstrates

Three files, mirroring what every other plugin in this tree looks like:
...
- **`plugin_initialize`** wraps its body in `try { } FC_LOG_AND_RETHROW()`, which is how a configuration
  failure is logged and re-raised so appbase aborts startup.

## Generating a copy

The repository ships a [plop](https://plopjs.com) generator, `create-cxx-plugin`, registered in
`plopfile.js`. It is the path the header comment points at, and it produces the same shape plus a test
target:
...
Copy the directory, rename the three files and every `template_plugin` occurrence inside them, then wire it
up:
...
A plugin that lives outside this tree does not need steps 1 and 3: `programs/nodeop/CMakeLists.txt` accepts a
`SYSIO_ADDITIONAL_PLUGINS` list of external source directories, adds each as a subdirectory, and links every
target the directory reports through the `sysio_additional_plugin` macro.
