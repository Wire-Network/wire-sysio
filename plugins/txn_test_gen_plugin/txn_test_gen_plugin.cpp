...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     sysio::detail::txn_test_gen_status result = { status };

#define INVOKE_V_R_R(api_handle, call_name, in_param0, in_param1) \
     const auto& vs = fc::json::json::from_string(body).as<fc::variants>(); \
     api_handle->call_name(vs.at(0).as<in_param0>(), vs.at(1).as<in_param1>()); \
     sysio::detail::txn_test_gen_empty result;
...
struct txn_test_gen_plugin_impl {
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(contracts::sysio_token_abi().data()).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer sysio_token_serializer{fc::json::from_string(contracts::sysio_token_abi().data()).as<abi_def>(),
         ...
         //create some test accounts
         {
            ...
            //create "A" account
            {
            auto owner_auth   = sysio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            auto active_auth  = sysio::chain::authority{1, {{txn_text_receiver_A_pub_key, 1}}, {}};
            ...
            //create "B" account
            {
            auto owner_auth   = sysio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            auto active_auth  = sysio::chain::authority{1, {{txn_text_receiver_B_pub_key, 1}}, {}};
            ...
            //create "T" account
            {
            auto owner_auth   = sysio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
            auto active_auth  = sysio::chain::authority{1, {{txn_text_receiver_C_pub_key, 1}}, {}};
         ...
         //set newaccountT contract to sysio.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = contracts::sysio_token_wasm();
...

            {
               setabi handler;
               handler.account = newaccountT;
               handler.abi = fc::raw::pack(json::from_string(contracts::sysio_token_abi().data()).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountT,name("active")}}, handler);
            }

            {
               ...
               act.data = sysio_token_serializer.variant_to_binary("create",
            ...
            }
            {
               ...
               act.name = "issue"_n;
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = sysio_token_serializer.variant_to_binary("issue",
            ...
            }
            {
               ...
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = sysio_token_serializer.variant_to_binary("transfer",
   ...
   string start_generation(const std::string& salt, const uint64_t& period, const uint64_t& batch_size) {
      ...
      auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();
      abi_serializer sysio_token_serializer{fc::json::from_string(contracts::sysio_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time )};
      ...
      act_a_to_b.data = sysio_token_serializer.variant_to_binary("transfer",
      ...
      act_b_to_a.data = sysio_token_serializer.variant_to_binary("transfer",
                                                                  fc::json::from_string(fc::format_string("{\"from\":\"${from}\",\"to\":\"${to}\",\"quantity\":\"1.0000 CUR\",\"memo\":\"${l}\"}",
