...
public:
    ...
    */
   signature_provider& create_provider_for_spec(const std::string& spec);

   /**
    * Create a known signature provider
    *
    * @param key_name explicit key_name to use
    * @param spec string spec described in source
    * @return a signature provider
    */
   signature_provider& create_provider_for_spec(const std::string& key_name, const std::string& spec);
