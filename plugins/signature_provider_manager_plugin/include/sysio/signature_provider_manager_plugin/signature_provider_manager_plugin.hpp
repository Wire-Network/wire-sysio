 ...
 * `signature_provider_ssm_plugin`) that registers its handler from its
 * CONSTRUCTOR via `sigprov::register_scheme_handler` -- the manager then
 * creates the configured providers at its own init, gated on the plugin
 * being enabled via `--plugin`. Host applications that do not use the plugins
 * may instead register directly on the manager instance via
 * `signature_provider_manager_plugin::register_spec_handler(scheme, handler)`
 * (ungated). The built-in `KEY:` and `KIOD:` schemes are not registrable --
 * they are handled directly by the plugin.
 */
 ...
 * `register_default_signature_providers`) asserts it is called before this
 * plugin has started; all mutation happens on the main thread during the
 * sequential initialize/startup phases. Runtime access from other threads is
 * read-only and therefore needs no synchronization.
 */
...
public:
   ...
   /**
    * Plugin initialize hook. Creates every configured `--signature-provider`
    * provider eagerly and synchronously:
    *
    ...
    *     error.
    *
    * Because every provider consumer `APPBASE_PLUGIN_REQUIRES` this plugin,
    * this init runs before any of them -- so the full provider set exists by
    * the time chain_plugin / producer_plugin / the outposts look, independent
    * of `--plugin` ordering.
    */
    ...
    * Plugin startup hook.
    *
    * The startup-probe pass: any provider whose `spec_handler` supplied a
    * `startup_probe` in its `provider_spec_result` (today this is only the
    * KMS handler, which issues a free `GetPublicKey` to validate credentials,
    * region, IAM, and pinned key) has that probe invoked here. Aborts startup
    * loudly on permanent misconfiguration; transient errors are logged and
    * deferred to the first sign, so an AWS blip at restart never blocks a
    * boot. Attaching a probe IS the opt-in -- there is deliberately no enable
    * flag. A no-op when no probes were attached.
    */
   void plugin_startup();
    ...
    * directly on this instance -- the explicit host-application / test path.
    *
    * Built-in schemes (`KEY:`, `KIOD:`) are not registrable -- attempting to
    * register them throws. Each non-built-in scheme may be registered at most
    * once. A handler registered here is ungated: matching specs create
    * eagerly at `plugin_initialize` with no `--plugin` check. Call it before
    * `app().initialize(...)` so it is in place when the manager parses the
    ...
    *
    * `register_plugin<>` (static, used elsewhere in main()) only enqueues the
    * plugin name; use `_register_plugin<>` (instance method, idempotent) to
    * construct or fetch the instance first. Provider PLUGINS do not use this
    * -- they register in the process-wide `sigprov` registry from their
    * constructor (`sigprov::register_scheme_handler`), which the manager gates
    * on `--plugin`.
    *
...
/**
 * A construction-registered handler for a `<provider-type>:` scheme, plus the
...
/**
 * Register a provider plugin's scheme handler with the manager, from the
 * plugin's CONSTRUCTOR.
 *
 * This is what makes provider ordering a non-issue. appbase constructs every
 * registered plugin before it initializes any, so a handler registered here
 * is present by the time the manager's `plugin_initialize` creates providers
 * -- and the manager is `APPBASE_PLUGIN_REQUIRES`-d by every consumer
 * (chain_plugin, producer_plugin, the outposts), so its init runs before
 * theirs. The manager therefore creates every configured provider at its own
 * init, before any consumer, regardless of where the provider plugin's
 * `--plugin` line sits in the config.
 *
 * The handler is a free function (no plugin instance needed). Registering is
 * cheap and side-effect-free -- no AWS, no network; whether the scheme is
 * actually engaged is gated separately by the manager on the `--plugin`
 * enable list. Process-wide static storage (a fact about the binary, not any
 * one application instance), idempotent via insert-or-assign because test
 * binaries construct plugins repeatedly across scoped_app instances.
 * Deliberately unsynchronized: writes (plugin construction) and reads (the
 * manager's init) all happen on the main thread in sequential appbase
 * lifecycle phases.
 *
...
/**
 * The construction-registered entry for @p scheme, or nullptr if no linked
 * provider plugin provides it. The returned pointer is valid for the process
