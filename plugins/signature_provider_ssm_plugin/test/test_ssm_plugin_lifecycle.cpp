...
// ---------------------------------------------------------------------------
// Plugin wiring: what constructing signature_provider_ssm_plugin does. All
...
// (test_extension_schemes.cpp); the create-runs-with-the-real-handler proof is
// a nodeop boot smoke, since a real SSM creation performs the Parameter Store
// fetch.
// ---------------------------------------------------------------------------
...
BOOST_AUTO_TEST_CASE(constructor_registers_scheme_handler) {
   ...
   // The plugin's constructor registered the SSM handler in the process-wide
   ...
   // on. (Construction happens as appbase builds the registered plugins.)
