\chapter{FAQ}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2faq}{}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2faq}\index{FAQ@{FAQ}}
\doxysection*{General}
...
Rapid\+JSON is a C++ library for parsing and generating JSON. You may check all \doxylink{features_8md}{features} of it.
...
Yes. \doxylink{struct_a}{A} simple executable which parses a JSON and prints its statistics is less than 30KB on Windows.
...
Yes. It is deployed in both client and server real applications. \doxylink{struct_a}{A} community member reported that Rapid\+JSON in their system parses 50 million JSONs daily.
...
{\itshape in situ} parsing decodes the JSON strings directly into the input JSON. This is an optimization which can reduce memory consumption and improve performance, but the input JSON will be modified. Check \doxylink{dom_8md}{in-\/situ parsing} for details.
\item When does parsing generate an error?

The parser generates an error when the input JSON contains invalid syntax, or a value can not be represented (a number is too big), or the handler of parsers terminate the parsing. Check \doxylink{dom_8md}{parse error} for details.
\item What error information is provided?

The error is stored in {\ttfamily \doxylink{struct_parse_result}{Parse\+Result}}, which includes the error code and offset (number of characters from the beginning of JSON). The error code can be translated into human-\/readable error message.
...
\doxylink{struct_call}{Call} one of the {\ttfamily Set\+XXX()} methods -\/ they call destructor which deallocates DOM data\+:
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\mbox{\hyperlink{yubihsm__pkcs11_8c_a107648633008a7f2d872b4b3d98c2f96}{d}}.Swap(\mbox{\hyperlink{document_8h_a071cf97155ba72ac9a1fc4ad7e63d481}{Value}}(\mbox{\hyperlink{rapidjson_8h_a1d1cfd8ffb84e947f82999c682b666a7a146f46700e905e8df96a6a90b5c7640f}{kObjectType}}).Move());\ }
...
\doxysection*{Document/\+Value (DOM)}
...
There are two APIs\+: constructor with allocator, and {\ttfamily Copy\+From()}. See \doxylink{fc_2include_2fc_2crypto_2webauthn__json_2doc_2tutorial_8md}{Deep Copy Value} for an example.
...
When using {\ttfamily Get\+Int()}, {\ttfamily Get\+Uint()}, ... conversion may occur. For integer-\/to-\/integer conversion, it only convert when it is safe (otherwise it will assert). However, when converting a 64-\/bit signed/unsigned integer to double, it will convert but be aware that it may lose precision. \doxylink{struct_a}{A} number with fraction, or an integer larger than 64-\/bit, can only be obtained by {\ttfamily Get\+Double()}.
\end{DoxyEnumerate}

\doxysection*{Reader/\+Writer (SAX)}


\begin{DoxyEnumerate}
\item Why don\textquotesingle{}t we just {\ttfamily printf} a JSON? Why do we need a {\ttfamily \doxylink{class_writer}{Writer}}?

Most importantly, {\ttfamily \doxylink{class_writer}{Writer}} will ensure the output JSON is well-\/formed. Calling SAX events incorrectly (e.\+g. {\ttfamily Start\+Object()} pairing with {\ttfamily End\+Array()}) will assert. Besides, {\ttfamily \doxylink{class_writer}{Writer}} will escapes strings (e.\+g., {\ttfamily \textbackslash{}n}). Finally, the numeric output of {\ttfamily printf()} may not be a valid JSON number, especially in some locale with digit delimiters. And the number-\/to-\/string conversion in {\ttfamily \doxylink{class_writer}{Writer}} is implemented with very fast algorithms, which outperforms than {\ttfamily printf()} or {\ttfamily iostream}.
...
Yes. It fully support UTF-\/8, UTF-\/16 (LE/\+BE), UTF-\/32 (LE/\+BE) and \doxylink{struct_a_s_c_i_i}{ASCII}.
...
Yes, use {\ttfamily \doxylink{struct_a_s_c_i_i}{ASCII}\texorpdfstring{$<$}{<}\texorpdfstring{$>$}{>}} as output encoding template parameter in {\ttfamily \doxylink{class_writer}{Writer}} can enforce escaping those characters.
\end{DoxyEnumerate}
...
User can use {\ttfamily \doxylink{class_file_read_stream}{File\+Read\+Stream}} to read the file chunk-\/by-\/chunk. But for {\itshape in situ} parsing, the whole file must be loaded.
\item Can I parse JSON while it is streamed from network?

Yes. User can implement a custom stream for this. Please refer to the implementation of {\ttfamily \doxylink{class_file_read_stream}{File\+Read\+Stream}}.
\item I don\textquotesingle{}t know what encoding will the JSON be. How to handle them?

You may use {\ttfamily \doxylink{class_auto_u_t_f_input_stream}{Auto\+UTFInput\+Stream}} which detects the encoding of input stream automatically. However, it will incur some performance overhead.
...
Rapid\+JSON\textquotesingle{}s {\ttfamily \doxylink{class_encoded_input_stream}{Encoded\+Input\+Stream}} can detect/consume BOM. {\ttfamily \doxylink{class_encoded_output_stream}{Encoded\+Output\+Stream}} can optionally write a BOM. See \doxylink{stream_8md}{Encoded Streams} for example.
...
Milo Yip (\href{https://github.com/miloyip}{\texttt{ miloyip}}) is the original author of Rapid\+JSON. Many contributors from the world have improved Rapid\+JSON. Philipp \doxylink{struct_a}{A}. Hartmann (\href{https://github.com/pah}{\texttt{ pah}}) has implemented a lot of improvements, setting up automatic testing and also involves in a lot of discussions for the community. Don Ding (\href{https://github.com/thebusytypist}{\texttt{ thebusytypist}}) implemented the iterative parser. Andrii Senkovych (\href{https://github.com/jollyroger}{\texttt{ jollyroger}}) completed the CMake migration. Kosta (\href{https://github.com/Kosta-Github}{\texttt{ Kosta-\/\+Github}}) provided a very neat short-\/string optimization. Thank you for all other contributors and community members as well.
...
\item Why did the repository move from Google \doxylink{struct_code}{Code} to Git\+Hub?
