...
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2readme_8zh-cn}{}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2readme_8zh-cn}\index{readme.zh-\/cn@{readme.zh-\/cn}}




\doxysubsection*{高效的 C++ JSON 解析／生成器，提供 SAX 及 DOM 风格 API}
...
\item Rapid\+JSON 对 Unicode 友好。它支持 UTF-\/8、\+UTF-\/16、\+UTF-\/32 (大端序／小端序)，并内部支持这些编码的检测、校验及转码。例如，\+Rapid\+JSON 可以在分析一个 UTF-\/8 文件至 DOM 时，把当中的 JSON 字符串转码至 UTF-\/16。它也支持代理对（surrogate pair）及 {\ttfamily "{}\textbackslash{}u0000"{}}（空字符）。
\end{DoxyItemize}

在 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2features_8zh-cn}{这里}{0} 可读取更多特点。
...
\doxysubsection*{v1.\+1 中的亮点 (2016-\/8-\/25)}


\begin{DoxyItemize}
\item 加入 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2pointer_8zh-cn}{JSON Pointer}{0} 功能，可更简单地访问及更改 DOM。
\item 加入 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2schema_8zh-cn}{JSON Schema}{0} 功能，可在解析或生成 JSON 时进行校验。
\item 加入 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2dom_8zh-cn}{放宽的 JSON 语法}{0} （注释、尾随逗号、\+Na\+N/\+Infinity）
\item 使用 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2tutorial_8zh-cn}{C++11 范围 for 循环}{0} 去遍历 array 和 object。
\item 在 x86-\/64 架构下，缩减每个 {\ttfamily Value} 的内存开销从 24 字节至 16 字节。
\end{DoxyItemize}

其他改动请参考 \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2_c_h_a_n_g_e_l_o_g}{change log}{0}.
...
成功生成后，你会在 {\ttfamily bin} 的目录下找到编译后的测试及例子可执行文件。而生成的文档将位于 build 下的 {\ttfamily doc/html} 目录。要执行测试，请在 build 下执行 {\ttfamily make test} 或 {\ttfamily ctest}。使用 {\ttfamily ctest -\/\doxylink{struct_v}{V}} 命令可获取详细的输出。

我们也可以把程序库安装至全系统中，只要在具管理权限下从 build 目录执行 {\ttfamily make install} 命令。这样会按系统的偏好设置安装所有文件。当安装 Rapid\+JSON 后，其他的 CMake 项目需要使用它时，可以通过在 {\ttfamily CMake\+Lists.\+txt} 加入一句 {\ttfamily find\+\_\+package(\+Rapid\+JSON)}。
...
\item \href{https://github.com/Tencent/rapidjson/blob/master/example/pretty/pretty.cpp}{\texttt{ pretty}}\+: 为 JSON 加入缩进与换行的命令行工具，当中使用了 {\ttfamily \doxylink{class_pretty_writer}{Pretty\+Writer}}。
...
\item \href{https://github.com/Tencent/rapidjson/blob/master/example/jsonx/jsonx.cpp}{\texttt{ jsonx}}\+: 实现了一个 {\ttfamily \doxylink{class_jsonx_writer}{Jsonx\+Writer}}，它能把 SAX 事件写成 \href{https://www-01.ibm.com/support/knowledgecenter/SS9H2Y_7.1.0/com.ibm.dp.doc/json_jsonx.html}{\texttt{ JSONx}}（一种 XML）格式。这个例子是把 JSON 输入转换成 JSONx 格式的命令行工具。
\end{DoxyItemize}
\item \doxylink{class_schema}{Schema} API
\begin{DoxyItemize}
\item \href{https://github.com/Tencent/rapidjson/blob/master/example/schemavalidator/schemavalidator.cpp}{\texttt{ schemavalidator}}\+: 使用 JSON \doxylink{class_schema}{Schema} 去校验 JSON 的命令行工具。
\end{DoxyItemize}
\item 进阶
\begin{DoxyItemize}
