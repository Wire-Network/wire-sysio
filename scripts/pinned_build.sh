...
if [ $# -eq 0 ] || [ -z "$1" ]
   then
      echo "Please supply a directory for the build dependencies to be placed and a directory for wire_sysio build and a value for the number of jobs to use for building."
      echo "The binary packages will be created and placed into the wire_sysio build directory."
      echo "./pinned_build.sh <dependencies directory> <wire_sysio build directory> <1-100>"
      exit -1
fi
