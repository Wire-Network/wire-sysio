...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sysio/node_$EOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/sysio/node_$EOSIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
