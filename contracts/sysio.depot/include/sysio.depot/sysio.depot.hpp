...
public:
   ...
   ACTION echotest(const fc::crypto::chain_kind_t& kind);
   ACTION reservedelta(const fc::crypto::chain_kind_t& kind, const asset& delta);

   ACTION swapquote(const fc::crypto::chain_kind_t& source_chain, const asset& source_amount,
                    const fc::crypto::chain_kind_t& target_chain);
...
private:
   ...
   TABLE reserve_s {
      ...
      chain_kind_t by_chain() const { return chain; }
   ...
   using reserve_t = multi_index<
      ...
      indexed_by<"bychain"_n,
                 const_mem_fun<reserve_s, chain_kind_t, &reserve_s::by_chain>>,
   ...
   TABLE swap_s {
      ...
      chain_kind_t by_source_chain() const { return source_chain; }

      chain_kind_t by_target_chain() const { return target_chain; }
      ...
      uint128_t by_source_chain_status_symbol() const {
         uint128_t compound_value = static_cast<uint128_t>(source_chain);
         compound_value <<= 96;
      ...
      uint128_t by_target_chain_status_symbol() const {
         uint128_t compound_value = static_cast<uint128_t>(target_chain);
         compound_value <<= 96;
         compound_value |= (static_cast<uint128_t>(status) << 64);
      ...
      uint128_t by_source_chain_symbol() const {
         uint128_t chain_symbol = static_cast<uint128_t>(source_chain);
         chain_symbol <<= 64;
   ...
   using swap_t = multi_index<
      ...
      indexed_by<"byschain"_n,
                 const_mem_fun<swap_s, chain_kind_t, &swap_s::by_source_chain>>,
      ...
      indexed_by<"bytchain"_n,
                 const_mem_fun<swap_s, chain_kind_t, &swap_s::by_target_chain>>,
   ...
   using msgchain_t = multi_index<
      ...
      indexed_by<"bychain"_n,
                 const_mem_fun<msgchain_s, chain_kind_t, &msgchain_s::by_chain>>,
      indexed_by<"bysymbol"_n,
