...
static const sysio::chain::name blockinfo_table_name = "blockinfo"_n;

// cspell:disable-next-line
static const sysio::chain::name blockinfo_tester_account_name = "binfotester"_n;
...
public:
                                     ...
                                     std::function<bool(block_info_record)> visitor) const
   {
      ...
      auto itr = kv_idx.lower_bound(boost::make_tuple(config::system_account_name, std::string_view(lo_key.data, 24)));
      for (; itr != kv_idx.end() && itr->code == config::system_account_name; ++itr) {
         auto kv = itr->key_view();
         if (kv.size() != 24) break;
         // Verify table and scope portions match
         if (kv.substr(0, 16) != std::string_view(lo_key.data, 16)) break;
   ...
   std::pair<std::optional<blockinfo_tester::latest_block_batch_info_result>, sysio::chain::transaction_trace_ptr>
   get_latest_block_batch_info(blockinfo_tester::get_latest_block_batch_info request)
   {
      ...
      fprintf(stderr, "DEBUG: action_traces.size() = %zu\n", action_traces.size());
      for (size_t idx = 0; idx < action_traces.size(); ++idx) {
         const auto& at = action_traces[idx];
         fprintf(stderr, "DEBUG:   trace[%zu]: receiver=%s account=%s name=%s ordinal=%u creator_ordinal=%u has_receipt=%d console=%s\n",
            idx, at.receiver.to_string().c_str(), at.act.account.to_string().c_str(), at.act.name.to_string().c_str(),
            at.action_ordinal.value, at.creator_action_ordinal.value,
            (int)at.receipt.has_value(), at.console.c_str());
         if (at.except.has_value()) {
            fprintf(stderr, "DEBUG:     except: %s\n", at.except->to_detail_string().c_str());
         }
      }
      if (result.second->except.has_value()) {
         fprintf(stderr, "DEBUG: trx except: %s\n", result.second->except->to_detail_string().c_str());
      }

      if (action_traces.size() < 3) {
         fprintf(stderr, "DEBUG: EARLY RETURN: action_traces.size() < 3\n");
         return result;
      }

      auto validate_action = [](const sysio::chain::action_trace& a, sysio::chain::name action_name) -> bool {
...
BOOST_AUTO_TEST_SUITE(sysio_system_blockinfo_tests)
