...
public:
   ...
   sysio_epoch_flushwtdw_tester() {
      ...
      create_accounts({
         ...
         CHALG_ACCOUNT, BATCHOP, UWRIT_OP,
   ...
   /// One-shot bootstrap: emissions config + t5 state + epoch config +
   /// opreg config + a bootstrapped batch op + a pending underwriter +
   /// an Ethereum outpost + initgroups + the genesis advance.
   ///
   ...
   abi_serializer sysio_abi, token_abi, epoch_abi, opreg_abi, msgch_abi;
};
