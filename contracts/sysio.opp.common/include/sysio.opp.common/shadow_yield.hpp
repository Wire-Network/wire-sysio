...
/// (the token contract therefore needs `sysio.code` on `from`'s active), and
/// advance the index by quantity / supply, carrying the remainder.
...
struct account {
   asset    balance;
   uint64_t index_checkpoint = 0;   ///< index value at the last settle of this row
...
struct yield_index {
   uint64_t index = 0;   ///< cumulative WIRE per shadow unit, scaled by YIELD_INDEX_SCALE
...
/// holder's pre-claim computation are one function.
inline uint64_t owed(const account& row, uint64_t index) {
   if (index <= row.index_checkpoint || row.balance.amount <= 0) return row.owed_wire;
   const u128 accrued = static_cast<u128>(row.balance.amount) * (index - row.index_checkpoint) / YIELD_INDEX_SCALE;
   return row.owed_wire + static_cast<uint64_t>(accrued);
}
