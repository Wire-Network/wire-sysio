...
Deployment therefore involves, in order: `setconfig` with the governance account; for each shadow token, the `sysio.code` grant above; and for each yield pool, `setyield` before the first tick.
