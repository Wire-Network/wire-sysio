...
{
    if(!(c >= '0' && c <= '9')) {
        sysio::check(false, std::string{"Invalid digit: "} + c);
    }
...
constexpr Int from_string(const char* s)
{
    ...
    // Additional check for hexadecimal without '0x' prefix
    const char* t = s;
    bool is_hex = false;
    while (const auto c = *t++) {
        if ((c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')) {
            is_hex = true;
            break;
        }
    ...
    if ((s[0] == '0' && s[1] == 'x') || is_hex)
    {
        if (s[0] == '0' && s[1] == 'x') s += 2;
        while (const auto c = *s++)
        {
            if (++num_digits > int{sizeof(x) * 2}) {
                sysio::check(false, std::string{"Integer overflow"});
            }
