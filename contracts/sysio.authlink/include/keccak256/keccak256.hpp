// keccak.hpp
// Copied and modified from @Jofrey's keccak256 library.
// Modified to work as a namespaced .hpp module. No compile and linking required.
// Github: https://github.com/XPRNetwork/eosio.evm.git
...
namespace keccak {
    constexpr std::array<uint8_t, 72> constants = { 1, 26, 94, 112, 31, 33, 121, 85, 14, 12, 53, 38, 63, 79, 93, 83, 82, 72, 22, 102, 121, 88, 33, 116,
    ...
    constexpr std::size_t BLOCK_SIZE = (1600 - 256 * 2) / 8;

    // constexpr uint64_t I64(uint64_t x) {
    //     return x##LL;
    // }

    constexpr uint64_t ROTL64(uint64_t qword, uint8_t n) {
        return (qword << n) ^ (qword >> (64 - n));
    }

    constexpr uint64_t le2me_64(uint64_t x) {
        return x;
    }

    inline bool IS_ALIGNED_64(const unsigned char* p) {
        return 0 == (7 & (p - static_cast<const unsigned char*>(0)));
    }

    inline void me64_to_le_str(void* to, const void* from, std::size_t length) {
        std::memcpy(to, from, length);
    }

    enum ConstType {
        TYPE_ROUND_INFO    = 0,
        TYPE_PI_TRANSFORM  = 24,
        TYPE_RHO_TRANSFORM = 48,
    };
    ...
    inline uint8_t getConstant(ConstType type, uint8_t index) {
        return constants[static_cast<uint8_t>(type) + index];
    }
