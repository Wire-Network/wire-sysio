...
namespace sysio {
  ...
  [[sysio::action]] void authlink::createlink(
    ...
    uint128_t name_chain = (static_cast<uint128_t>(username.value) << 64) | chainName.value;
    ...
    if (curve == "ec"_n) {
      ...
      std::string eth_addr = "0x" + sysio::to_hex(
         reinterpret_cast<const char*>(pk_bytes.data() + 12), 20);
      ...
      // Solana address
      auto sol = sysio::base58_encode(reinterpret_cast<const char*>(raw_key), 32);
      links.emplace(get_self(), [&](auto& a) {
          a.key      = links.available_primary_key();
          a.username = username;
          a.chain    = "solana"_n;
      ...
      std::string sui = "0x" + sysio::to_hex(
         reinterpret_cast<const char*>(sui_raw), 32);
  ...
  void authlink::onmanualrmv(const name& account, const name& permission) {
    ...
    switch(permission.value) {
      case "ed.ext"_n.value:
        ...
        name_curve = (static_cast<uint128_t>(account.value) << 64) | name("ed").value;
        break;
      case "ec.ext"_n.value:
        // Handle removal of EC curve link
        name_curve = (static_cast<uint128_t>(account.value) << 64) | name("ec").value;
        break;
      default:
  ...
  std::string authlink::pubkey_to_string(const sysio::public_key& pk) {
    switch (pk.index()) {
      case 3: { // PUB_EM_
        ...
        return "PUB_EM_" + sysio::base58_encode(reinterpret_cast<const char*>(buf.data()), buf.size());
      }
      ...
      case 4: { // PUB_ED_
        ...
        return "PUB_ED_" + sysio::base58_encode(reinterpret_cast<const char*>(buf.data()), buf.size());
      }
