...
void reserve::regreserve(sysio::slug_name chain_code,
   ...
   sysio::check(chain_precision <= WIRE_PRECISION,
                "chain_precision exceeds the depot frame (9) — the outpost must downscale to min(native, 9)");
...
void reserve::oncrtreserve(sysio::slug_name       chain_code,
   ...
   // chain_precision above the depot frame means a malformed attestation.
   if (chain_precision > WIRE_PRECISION) {
      sysio::print("oncrtreserve: chain_precision exceeds depot frame (9); skipping\n");
      return;
   }
