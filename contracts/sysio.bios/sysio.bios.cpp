...
void bios::setfinalizer( const finalizer_policy& finalizer_policy ) {
   ...
   sysio::finalizer_policy fin_policy;
   ...
   struct g1_equal {
      bool operator()(const sysio::bls_g1& lhs, const sysio::bls_g1& rhs) const {
         return std::memcmp(lhs.data(), rhs.data(), lhs.size()) == 0;
      }
   };
   std::unordered_set<sysio::bls_g1, g1_hash, g1_equal> unique_finalizer_keys;
   ...
   for (const auto& f: finalizer_policy.finalizers) {
      ...
      const auto pk = sysio::decode_bls_public_key_to_g1(f.public_key);
      // duplicate key check
      check(unique_finalizer_keys.insert(pk).second, "duplicate public key");

      const auto signature = sysio::decode_bls_signature_to_g2(f.pop);

      // proof of possession of private key check
      check(sysio::bls_pop_verify(pk, signature), "proof of possession failed");

      std::vector<char> pk_vector(pk.begin(), pk.end());
      fin_policy.finalizers.emplace_back(sysio::finalizer_authority{f.description, f.weight, std::move(pk_vector)});
   }
