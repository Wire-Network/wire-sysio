...
namespace sysiosystem {
   finalizer_auth_info::finalizer_auth_info(const finalizer_info& finalizer)
      ...
      , fin_authority( sysio::finalizer_authority{
   ...
   // Returns true if nodeop has transitioned to Savanna (having last proposed finalizers)
   ...
   static sysio::checksum256 get_finalizer_key_hash(const std::string& finalizer_key) {
   ...
   // from `proposed_finalizers` and calls sysio::set_finalizers host function
   ...
   void system_contract::set_proposed_finalizers( std::vector<finalizer_auth_info> proposed_finalizers ) {
      ...
      std::vector<sysio::finalizer_authority> finalizer_authorities;
