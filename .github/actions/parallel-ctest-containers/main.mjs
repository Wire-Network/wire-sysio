...
try {
   ...
   tests.forEach(t => {
      subprocesses.push(new Promise(resolve => {
         ...
         child_process.spawn("docker", ["run", "--security-opt", "seccomp=unconfined", "-e", "GITHUB_ACTIONS=True", "--name", cname, "--init", baseImage, "bash", "-c", `cd build; ctest --output-on-failure -R '^${t.name}$' --timeout ${test_timeout}`], {stdio:"inherit"}).on('close', code => resolve(code));
      }));
   });
