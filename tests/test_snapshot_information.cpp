...
BOOST_AUTO_TEST_CASE_TEMPLATE(test_snapshot_information, SNAPSHOT_SUITE, snapshot_suites) {
   ...
   BOOST_REQUIRE_EQUAL(block->block_num(), 8u); // ensure that test setup stays consistent with original snapshot setup
   // undo the auto-pending from tester
   chain.control->abort_block();

   auto block2 = chain.produce_block();
   BOOST_REQUIRE_EQUAL(block2->block_num(), 9u); // ensure that test setup stays consistent with original snapshot setup
   // undo the auto-pending from tester
   chain.control->abort_block();
   ...
   BOOST_REQUIRE_EQUAL(test_snap_info.head_block_num, 8u);
