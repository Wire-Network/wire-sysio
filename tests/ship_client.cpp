...
int main(int argc, char* argv[]) {
   ...
   try {
      ...
      while(num_requests--) {
         ...
         sysio::check(!result_doucment.HasParseError(),                                      "Failed to parse result JSON from abieos");
         sysio::check(result_doucment.IsArray(),                                             "result should have been an array (variant) but it's not");
         sysio::check(result_doucment.Size() == 2,                                           "result was an array but did not contain 2 items like a variant should");
         sysio::check(std::string(result_doucment[0].GetString()) == "get_status_result_v0", "result type doesn't look like get_status_result_v0");
         sysio::check(result_doucment[1].IsObject(),                                         "second item in result array is not an object");
         sysio::check(result_doucment[1].HasMember("head"),                                  "cannot find 'head' in result");
         sysio::check(result_doucment[1]["head"].IsObject(),                                 "'head' is not an object");
         sysio::check(result_doucment[1]["head"].HasMember("block_num"),                     "'head' does not contain 'block_num'");
         sysio::check(result_doucment[1]["head"]["block_num"].IsUint(),                      "'head.block_num' isn't a number");
