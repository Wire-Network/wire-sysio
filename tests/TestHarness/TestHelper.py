...
class TestHelper(object):
    ...
    DEFAULT_PORT=Utils.shardPort(8888)
    DEFAULT_WALLET_PORT=Utils.shardPort(9899)
    ...
    def createArgumentParser(includeArgs, applicationSpecificArgs=AppArgs(), suppressHelp: bool=False) -> argparse.ArgumentParser:
        ...
        if "--kill-count" in includeArgs:
            thGrp.add_argument("--kill-count", type=int, help=argparse.SUPPRESS if suppressHelp else "nodeop instances to kill", default=-1)
        ...
        if "--dump-error-details" in includeArgs:
            thGrp.add_argument("--dump-error-details",
                                     help=argparse.SUPPRESS if suppressHelp else "Upon error print etc/sysio/node_*/config.ini and <test_name><pid>/node_*/stderr.log to stdout",
                                     action='store_true')
