...
# ***   create accounts to vote in desired producers   ***
    ...
    accountsToCreate = [cluster.sysioAccount]
    ...
    for account in accounts:
        trans = node.createInitializeAccount(account, cluster.sysioAccount, nodeOwner=cluster.carlAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
    ...
    Print("Transfer funds to new accounts via %s" % (cluster.sysioAccount.name))
    for account in accounts:
        ...
        trans = node.transferFunds(cluster.sysioAccount, account, transferAmount, "test transfer", waitForTransBlock=False, reportStatus=False)
        checkTransIds.append(Node.getTransId(trans))
    ...
    Print("Transfer funds %s from account %s to %s" % (overdrawTransferAmount, overdrawAccount.name, cluster.sysioAccount.name))
    overdrawtrans = node.transferFunds(overdrawAccount, cluster.sysioAccount, overdrawTransferAmount, "test overdraw transfer", exitOnError=False, reportStatus=False, retry=1)
