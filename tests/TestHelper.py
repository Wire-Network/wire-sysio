...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="nodeop instances to kill", default=-1)
        ...
        if "--dump-error-details" in includeArgs:
            parser.add_argument("--dump-error-details",
                                     help="Upon error print etc/sysio/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                                     action='store_true')
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all nodeop and kleos instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates nodeop and kleos are in path and can be started up.", action='store_true')
