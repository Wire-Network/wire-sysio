...
try:
    ...
    if dontLaunch: # run test against remote cluster
        ...
        specificExtraNodeopArgs[total_nodes-1] = f' --read-mode head '
        if activateIF: # irreversible mode speculative trx execution not recommended in legacy mode
            specificExtraNodeopArgs[total_nodes-2] = f' --read-mode irreversible '
        specificExtraNodeopArgs[total_nodes-3] = f' --read-mode speculative '

        if cluster.launch(pnodes=pnodes, totalNodes=total_nodes, topo=topo, delay=delay, specificExtraNodeopArgs=specificExtraNodeopArgs, activateIF=activateIF) is False:
