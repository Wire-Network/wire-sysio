   ...
   void check_range_present(uint32_t first, uint32_t last) {
      ...
      if(enable_read) {
         for(auto i = first; i <= last; i++) {
            std::optional<sysio::state_history::ship_log_entry> entry = log->get_entry(i);
            BOOST_REQUIRE(!!entry);
...
BOOST_AUTO_TEST_CASE(empty) { try {
   ...
   {
      sysio::state_history::state_history_log log(log_dir.path()/ "empty");
      BOOST_REQUIRE(log.empty());
   }
   //reopen
   {
   ...
   {
      sysio::state_history::state_history_log log(log_dir.path() / "empty", state_history::state_history_log::no_non_local_get_block_id_func, simple_prune_conf);
      BOOST_REQUIRE(log.empty());
   }
   {
      sysio::state_history::state_history_log log(log_dir.path() / "empty", state_history::state_history_log::no_non_local_get_block_id_func, simple_prune_conf);
...
BOOST_DATA_TEST_CASE(basic, bdata::make({2u, 333u, 578'000u, 3'123'456'789u}) ^ bdata::make({102u, 400u, 578'111u, 3'123'456'900u}), start, end) try {
   ...
   sysio::state_history::log_catalog lc(tmpdir.path(), std::monostate(), "testlog");
...
BOOST_AUTO_TEST_CASE(regen_index) try {
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), std::monostate(), "empty");
      BOOST_REQUIRE(lc.empty());
   }
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), std::monostate(), "newlog");
      BOOST_REQUIRE(lc.empty());
...
BOOST_DATA_TEST_CASE(basic_split, boost::unit_test::data::make({5u, 6u, 7u, 8u, 9u, 10u, 578'000u, 3'123'456'789u}) *
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), conf, "splitit");
      BOOST_REQUIRE(lc.empty());
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), std::monostate(), "splitit");
   ...
   BOOST_REQUIRE_EXCEPTION(sysio::state_history::log_catalog(tmpdir.path(), conf, "splitit"),
                           plugin_exception,
...
BOOST_DATA_TEST_CASE(split_forks, bdata::xrange(1u, 6u), fork_size) try {
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), conf, "logz");
      BOOST_REQUIRE(lc.empty());
...
BOOST_AUTO_TEST_CASE(old_log_format) try {
...

   {
      ...
      for(unsigned blocknum = begin_block; blocknum < end_block; ++blocknum) {
         ...
         bio::filtering_ostreambuf output(bio::zlib_compressor() | sysio::detail::counter() | bio::restrict(file.seekable_device(), insertpos + raw::pack_size(legacy_header)));
         bio::copy(hashed_randomness, output);
         wrote_data_for_blocknum[blocknum] = hashed_randomness.component<sha256_filter>(0)->enc->result();
         legacy_header.first.payload_size = output.component<sysio::detail::counter>(1)->characters() + sizeof(decltype(legacy_header.second));
...

   {
      ...
      sysio::state_history::log_catalog lc(tmpdir.path(), std::monostate(), "old");
...
BOOST_DATA_TEST_CASE(rewrite_same, bdata::make(log_configs_for_rewrite_same), config) try {
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), config, "mr,log");
      for(unsigned i = begin_block; i < end_block; ++i)
         lc.pack_and_write_entry(fake_blockid_for_num(i), fake_blockid_for_num(i-1), [&](bio::filtering_ostreambuf& obuf) {
...
BOOST_DATA_TEST_CASE(rewrite_same_remembered, bdata::make(log_configs_for_rewrite_remembered), config) try {
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), config, "huh");
      for(unsigned i = begin_block; i < end_block; ++i)
         lc.pack_and_write_entry(fake_blockid_for_num(i), fake_blockid_for_num(i-1), [&](bio::filtering_ostreambuf& obuf) {
            bio::filtering_istreambuf hashed_randomness(sha256_filter() | bio::restrict(random_source(), 0, 64*1024));
            bio::copy(hashed_randomness, obuf);
...
BOOST_DATA_TEST_CASE(clear, bdata::make(log_configs_for_clear) * bdata::make({9u, 10u, 11u}), config, after_clear_begin_block) try {
   ...
   {
      sysio::state_history::log_catalog lc(tmpdir.path(), config, "clearme");
