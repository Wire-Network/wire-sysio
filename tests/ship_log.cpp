...
struct ship_log_fixture {
   ...
   void check_not_present(uint32_t index) {
      sysio::state_history_log_header header;
      BOOST_REQUIRE_EXCEPTION(log->get_entry(index, header), sysio::chain::plugin_exception, [](const sysio::chain::plugin_exception& e) {
...
private:
   void bounce() {
      ...
      std::optional<sysio::state_history_log_prune_config> prune_conf;
      if(prune_blocks) {
         prune_conf.emplace();
...
BOOST_AUTO_TEST_CASE(empty) { try {
   ...
   {
      sysio::state_history_log log("empty", log_file.path().string(), index_file.path().string());
      BOOST_REQUIRE_EQUAL(log.begin_block(), log.end_block());
   }
   //reopen
   {
   ...
   const sysio::state_history_log_prune_config simple_prune_conf = {
      .prune_blocks = 4
   };
   {
      sysio::state_history_log log("empty", log_file.path().string(), index_file.path().string(), simple_prune_conf);
      BOOST_REQUIRE_EQUAL(log.begin_block(), log.end_block());
   }
   {
      sysio::state_history_log log("empty", log_file.path().string(), index_file.path().string(), simple_prune_conf);
