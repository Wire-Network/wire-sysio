...
class Cluster(object):
    ...
    # walletd [True|False] Is kiod running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        """Cluster container.
        walletd [True|False] Is wallet kiod running. If not load the wallet plugin
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodeopArgs="", useBiosBootFile=True, specificExtraNodeopArgs=None, onlySetProds=False,
        ...
        extraNodeopArgs: string of arguments to pass through to each nodoes instance (via --nodeop flag on launcher)
        ...
        loadSystemContract: indicate whether the sysio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.EosLauncherPath, pnodes, totalNodes, delay, datetime.datetime.now(datetime.UTC).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodeopArgs="--resource-monitor-not-shutdown-on-threshold-exceeded --max-transaction-time -1 --abi-serializer-max-time-ms 990000 --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodeopArgs += " --plugin sysio::wallet_api_plugin"
        if Utils.Debug:
        ...
        if nodeopArgs:
            cmdArr.append("--nodeop")
            cmdArr.append(nodeopArgs)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodeop")
                cmdArr.append(path)
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse sysio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by sysio-launcher."""

        Utils.Print("Starting cluster bootstrap using bios_boot.sh.")
        assert PFSetupPolicy.isValid(pfSetupPolicy)

        cmd="bash bios_boot.sh"
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/sysio.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/sysio.bios"
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"sysio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init sysio transfer")
            opts="--permission sysio@active"
            if name != "sysio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from sysio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        Utils.Print("Starting cluster bootstrap. (xx)")
        assert PFSetupPolicy.isValid(pfSetupPolicy)
        ...
        trans=biosNode.publishContract(sysioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        roa = sysioAccount.clone()
        roa.name = "sysio.roa"
        trans = biosNode.createAccount(roa, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (roa.name))
            return None
        ...
        trans=biosNode.createAccount(sysioTokenAccount, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (sysioTokenAccount.name))
            return None

        sysioRamAccount=copy.deepcopy(sysioAccount)
        sysioRamAccount.name="sysio.ram"
        trans=biosNode.createAccount(sysioRamAccount, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (sysioRamAccount.name))
            return None

        sysioRamfeeAccount=copy.deepcopy(sysioAccount)
        sysioRamfeeAccount.name="sysio.ramfee"
        trans=biosNode.createAccount(sysioRamfeeAccount, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (sysioRamfeeAccount.name))
            return None

        sysioStakeAccount=copy.deepcopy(sysioAccount)
        sysioStakeAccount.name="sysio.stake"
        trans=biosNode.createAccount(sysioStakeAccount, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (sysioStakeAccount.name))
            return None

        sysioRoaAccount=copy.deepcopy(sysioAccount)
        sysioRoaAccount.name="sysio.roa"
        trans=biosNode.createAccount(sysioRoaAccount, sysioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (sysioRoaAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(sysioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None
    ...
    def killall(self, kill=True, silent=True, allInstances=False):
        """Kill cluster nodeop instances. allInstances will kill all nodeop instances running on the system."""
