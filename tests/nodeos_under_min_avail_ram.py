...
try:
    ...
    maxRAMValue=1010
    extraNodeosArgs=" %s %d %s %d  --http-max-response-time-ms 990000 --plugin sysio::trace_api_plugin --trace-no-abis " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to sysio contract for %d consecutive times, looks like nodeos already exited." % (timeOutCount))
