...
###############################################################################################
# This test starts nodeop process which is configured with GELF logging endpoint as localhost, 
...
# It is good to have at least one integration test that does not use sysio::chain_api_plugin or sysio::http_plugin
...
nodeop = Node(TestHelper.LOCAL_HOST, TestHelper.DEFAULT_PORT, node_id, config_dir, data_dir, start_nodeop_cmd, unstarted=True)
...
try:
  ...
  nodeop.launchUnstarted(waitForAlive=False)
  time.sleep(nodeop_run_time_in_sec)
finally:
   cleanup()
