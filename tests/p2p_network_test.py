...
# sysio should have the same key as defproducera
sysio = copy.copy(defproduceraAccount)
sysio.name = "sysio"
...
trans=node0.publishContract(sysio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish sysio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, sysio)
