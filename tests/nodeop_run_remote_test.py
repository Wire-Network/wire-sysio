...
###############################################################
# nodeop_run_remote_test
#
#  Tests remote capability of the nodeop_run_test. Test will setup cluster and pass nodes info to nodeop_run_test. E.g.
#  nodeop_run_remote_test.py -v --clean-run --dump-error-detail
#
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodeop_run_test.py"
...
try:
    ...
    if cluster.launch(pnodes=pnodes, totalNodes=total_nodes, prodCount=prodCount, topo=topo, delay=delay, onlyBios=onlyBios, extraNodeopArgs=traceNodeopArgs) is False:
    ...
    Print("Starting up %s test: %s" % ("nodeop", actualTest))
    Print("cmd: %s\n" % (cmd))
