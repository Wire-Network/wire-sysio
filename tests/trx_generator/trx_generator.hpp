...
namespace sysio::testing {
   ...
   struct signed_transaction_w_signer {
      signed_transaction_w_signer(sysio::chain::signed_transaction trx, fc::crypto::private_key key) : _trx(std::move(trx)), _signer(key) {}

      sysio::chain::signed_transaction _trx;
      fc::crypto::private_key _signer;
   };

   struct action_pair_w_keys {
      action_pair_w_keys(sysio::chain::action first_action, sysio::chain::action second_action, fc::crypto::private_key first_act_signer, fc::crypto::private_key second_act_signer)
      ...
      sysio::chain::action _first_act;
      sysio::chain::action _second_act;
   ...
   struct trx_generator_base_config {
      ...
      sysio::chain::chain_id_type _chain_id = sysio::chain::chain_id_type::empty_chain_id();
      sysio::chain::name _contract_owner_account = sysio::chain::name();
      fc::microseconds _trx_expiration_us = fc::seconds(3600);
      sysio::chain::block_id_type _last_irr_block_id = sysio::chain::block_id_type();
      std::string _log_dir = ".";
   ...
   struct accounts_config {
      std::vector<sysio::chain::name> _acct_name_vec;
   ...
   struct trx_generator_base {
      ...
      virtual void update_resign_transaction(sysio::chain::signed_transaction& trx, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix, uint64_t& nonce,
                                     const fc::microseconds& trx_expiration, const sysio::chain::chain_id_type& chain_id, const sysio::chain::block_id_type& last_irr_block_id);

      void push_transaction(signed_transaction_w_signer& trx, uint64_t& nonce_prefix,
                            uint64_t& nonce, const fc::microseconds& trx_expiration, const sysio::chain::chain_id_type& chain_id,
                            const sysio::chain::block_id_type& last_irr_block_id);

      void set_transaction_headers(sysio::chain::transaction& trx, const sysio::chain::block_id_type& last_irr_block_id, const fc::microseconds& expiration, uint32_t delay_sec = 0);

      signed_transaction_w_signer create_trx_w_actions_and_signer(std::vector<sysio::chain::action>&& act, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix,
                                                                  uint64_t& nonce, const fc::microseconds& trx_expiration, const sysio::chain::chain_id_type& chain_id,
                                                                  const sysio::chain::block_id_type& last_irr_block_id);

      void log_first_trx(const std::string& log_dir, const sysio::chain::signed_transaction& trx);
   ...
   struct transfer_trx_generator : public trx_generator_base {
      ...
      sysio::chain::bytes make_transfer_data(const sysio::chain::name& from, const sysio::chain::name& to, const sysio::chain::asset& quantity, const std::string& memo);
      auto make_transfer_action(sysio::chain::name account, sysio::chain::name from, sysio::chain::name to, sysio::chain::asset quantity, std::string memo);
   ...
   struct trx_generator : public trx_generator_base{
      ...
      sysio::chain::abi_serializer _abi;
      ...
      std::vector<sysio::chain::action> generate_actions();
      virtual void update_resign_transaction(sysio::chain::signed_transaction& trx, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix, uint64_t& nonce,
                                     const fc::microseconds& trx_expiration, const sysio::chain::chain_id_type& chain_id, const sysio::chain::block_id_type& last_irr_block_id);
