...
# ship_kv_delta_test
#
# Verifies that kv::raw_table (format=0) and multi_index (format=1)
# data flows correctly through the SHiP delta pipeline.
#
...
# 2. Deploys both a kv::raw_table contract and a multi_index contract
# 3. Pushes actions to both
# 4. Runs ship_streamer with --fetch-deltas
# 5. Verifies "contract_row" and "contract_row_kv" delta table
#    names appear in the SHiP binary output
###############################################################
...
try:
    ...
    # Deploy test_kv_map (format=0 / kv::raw_table)
    contractDir = os.path.join(os.getcwd(), "unittests", "test-contracts", "test_kv_map")
    Print("Deploy test_kv_map contract (format=0 / kv::raw_table)")
    ...
    # Deploy get_table_test (format=1 / multi_index)
    contractDir2 = os.path.join(os.getcwd(), "unittests", "test-contracts", "get_table_test")
    Print("Deploy get_table_test contract (format=1 / multi_index)")
    ...
    # Push kv::raw_table store actions (format=0)
    Print("Push kv::raw_table store actions")
    ...
    # Verify kv::raw_table data via get action
    Print("Verify kv::raw_table data reads back correctly")
    trxGet = {"actions": [{"account": "kvmap", "name": "get",
              "authorization": [{"actor": "kvmap", "permission": "active"}],
              "data": {"region": "us-east", "id": 1}}]}
    results = apiNode.pushTransaction(trxGet)
    assert results[0], f"kv::raw_table get failed: {results}"
    ...
    # "contract_row" (12 bytes) = hex: 0c636f6e74726163745f726f77
    contract_row_hex = "0c636f6e74726163745f726f77"

    found_contract_row = contract_row_hex in output
    found_contract_row_kv = contract_row_kv_hex in output

    Print(f"contract_row delta found: {found_contract_row}")
    Print(f"contract_row_kv delta found: {found_contract_row_kv}")

    assert found_contract_row, "Expected contract_row delta (format=1) not found in SHiP output"
    assert found_contract_row_kv, "Expected contract_row_kv delta (format=0) not found in SHiP output"

    Print("SUCCESS: Both contract_row and contract_row_kv deltas verified in SHiP output")
