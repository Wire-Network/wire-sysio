...
BOOST_FIXTURE_TEST_CASE( get_scope_test, validating_tester ) try {
   ...
   if (result.rows.size() >= 5u) {
      ...
      BOOST_REQUIRE_EQUAL(name("sysio.token"_n), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, validating_tester ) try {
   ...
   if (result.rows.size() >= 4u) {
      ...
      BOOST_REQUIRE_EQUAL("sysio.token", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("sysio.token", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("sysio.token", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("sysio.token", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_next_key_test, validating_tester ) try {
   ...
   // Secondary index queries are no longer supported (KV uses get_kv_rows API instead)
