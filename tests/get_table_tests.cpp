...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   // iterate over scope
   sysio::chain_apis::read_only plugin(*(this->control), {}, fc::microseconds::maximum(), {}, {});
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   sysio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   bidname("initc"_n, "io"_n, sysio::chain::asset::from_string("12.0000 SYS"));
   bidname("initd"_n, "html"_n, sysio::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
