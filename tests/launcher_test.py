...
# launcher-test
#
# Specifically tests using the bios bootstrap script that is created by sysio-launcher
#
###############################################################
...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.sysioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.sysioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.sysioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.sysioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.sysioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.sysioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.sysioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
