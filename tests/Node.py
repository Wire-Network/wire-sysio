...
class Node(object):
    ...
    def relaunch(self, chainArg=None, newChain=False, skipGenesis=True, timeout=Utils.systemWaitTimeout, addSwapFlags=None, cachePopen=False, nodeopPath=None, waitForTerm=True):
        ...
        if nodeopPath: splittedCmd[0] = nodeopPath
    ...
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("sysio", "activate", data, opts)
    ...
    # kill all existing nodeop in case lingering from previous test
    @staticmethod
    def killAllNodeop():
        # kill the eos server
