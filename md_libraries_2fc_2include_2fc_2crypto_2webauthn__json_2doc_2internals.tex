\chapter{Internals}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals}{}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals}\index{Internals@{Internals}}
This section records some design and implementation details.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Architecture}{}\doxysection{\texorpdfstring{Architecture}{Architecture}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Architecture}
\doxysubsection*{SAX and DOM}

The basic relationships of SAX and DOM is shown in the following UML diagram.



The core of the relationship is the {\ttfamily Handler} concept. From the SAX side, {\ttfamily Reader} parses a JSON from a stream and publish events to a {\ttfamily Handler}. {\ttfamily \doxylink{class_writer}{Writer}} implements the {\ttfamily Handler} concept to handle the same set of events. From the DOM side, {\ttfamily Document} implements the {\ttfamily Handler} concept to build a DOM according to the events. {\ttfamily Value} supports a {\ttfamily Value\+::\+Accept(Handler\&)} function, which traverses the DOM to publish events.

With this design, SAX is not dependent on DOM. Even {\ttfamily Reader} and {\ttfamily \doxylink{class_writer}{Writer}} have no dependencies between them. This provides flexibility to chain event publisher and handlers. Besides, {\ttfamily Value} does not depends on SAX as well. So, in addition to stringify a DOM to JSON, user may also stringify it to a XML writer, or do anything else.

\doxysubsection*{Utility Classes}
...
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Value}{}\doxysection{\texorpdfstring{Value}{Value}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Value}
{\ttfamily Value} (actually a typedef of {\ttfamily \doxylink{class_generic_value}{Generic\+Value}\texorpdfstring{$<$}{<}\doxylink{struct_u_t_f8}{UTF8}\texorpdfstring{$<$}{<}\texorpdfstring{$>$}{>}\texorpdfstring{$>$}{>}}) is the core of DOM API. This section describes the design of it.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_DataLayout}{}\doxysubsection{\texorpdfstring{Data Layout}{Data Layout}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_DataLayout}
{\ttfamily Value} is a \href{http://en.wikipedia.org/wiki/Variant_type}{\texttt{ variant type}}. In Rapid\+JSON\textquotesingle{}s context, an instance of {\ttfamily Value} can contain 1 of 6 JSON value types. This is possible by using {\ttfamily union}. Each {\ttfamily Value} contains two members\+: {\ttfamily union \doxylink{struct_data}{Data} data\+\_\+} and a{\ttfamily unsigned flags\+\_\+}. The {\ttfamily flags\+\_\+} indicates the JSON type, and also additional information.
...
\begin{longtabu}spread 0pt [c]{*{4}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \doxylink{struct_bool}{Bool}   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32-\/bit   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64-\/bit    }\\\cline{1-4}
\endfirsthead
\hline
\endfoot
\hline
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ String   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32-\/bit   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64-\/bit    }\\\cline{1-4}
\endhead
{\ttfamily Ch\texorpdfstring{$\ast$}{*} str}   &Pointer to the string (may own)   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Object   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32-\/bit   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64-\/bit    }\\\cline{1-4}
\endhead
{\ttfamily Member\texorpdfstring{$\ast$}{*} members}   &Pointer to array of members (owned)   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Array   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32-\/bit   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64-\/bit    }\\\cline{1-4}
\endhead
{\ttfamily Value\texorpdfstring{$\ast$}{*} values}   &Pointer to array of values (owned)   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
\end{DoxyItemize}\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Flags}{}\doxysubsection{\texorpdfstring{Flags}{Flags}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Flags}
...
String has two optional flags. {\ttfamily k\+Copy\+Flag} means that the string owns a copy of the string. {\ttfamily k\+Inline\+Str\+Flag} means using \doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}{Short-\/\+String Optimization}{2}.

Number is a bit more complicated. For normal integer values, it can contains {\ttfamily k\+Int\+Flag}, {\ttfamily k\+Uint\+Flag}, {\ttfamily k\+Int64\+Flag} and/or {\ttfamily k\+Uint64\+Flag}, according to the range of the integer. For numbers with fraction, and integers larger than 64-\/bit range, they will be stored as {\ttfamily double} with {\ttfamily k\+Double\+Flag}.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}{}\doxysubsection{\texorpdfstring{Short-\/\+String Optimization}{Short-\/\+String Optimization}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}
...
\doxylink{struct_a}{A} special technique is applied. Instead of storing the length of string directly, it stores (Max\+Chars -\/ length). This make it possible to store 11 characters with trailing {\ttfamily \textbackslash{}0}.

This optimization can reduce memory usage for copy-\/string. It can also improve cache-\/coherence thus improve runtime performance.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_InternalAllocator}{}\doxysection{\texorpdfstring{Allocator}{Allocator}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_InternalAllocator}
...
Note that {\ttfamily Malloc()} and {\ttfamily Realloc()} are member functions but {\ttfamily Free()} is static member function.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_MemoryPoolAllocator}{}\doxysubsection{\texorpdfstring{Memory\+Pool\+Allocator}{Memory\+Pool\+Allocator}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_MemoryPoolAllocator}
{\ttfamily \doxylink{class_memory_pool_allocator}{Memory\+Pool\+Allocator}} is the default allocator for DOM. It allocate but do not free memory. This is suitable for building a DOM tree.

Internally, it allocates chunks of memory from the base allocator (by default {\ttfamily \doxylink{class_crt_allocator}{Crt\+Allocator}}) and stores the chunks as a singly linked list. When user requests an allocation, it allocates memory from the following order\+:


\begin{DoxyEnumerate}
\item User supplied buffer if it is available. (See \doxylink{dom_8md}{User Buffer section in DOM})
...
\end{DoxyEnumerate}\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingOptimization}{}\doxysection{\texorpdfstring{Parsing Optimization}{Parsing Optimization}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingOptimization}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_SkipwhitespaceWithSIMD}{}\doxysubsection{\texorpdfstring{Skip Whitespaces with SIMD}{Skip Whitespaces with SIMD}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_SkipwhitespaceWithSIMD}
...
\doxylink{struct_a}{A} simple implementation will be simply\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{\textcolor{keywordtype}{void}\ \mbox{\hyperlink{reader_8h_a60338858b2582eca23f3e509a2d82e0e}{SkipWhitespace}}(InputStream\&\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}})\ \{}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordflow}{while}\ (\mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\ '}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)n'}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)r'}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)t'})}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Take();}
\DoxyCodeLine{\}}
...
To enable this optimization, need to define {\ttfamily RAPIDJSON\+\_\+\+SSE2}, {\ttfamily RAPIDJSON\+\_\+\+SSE42} or {\ttfamily RAPIDJSON\+\_\+\+NEON} before including {\ttfamily \doxylink{rapidjson_8h}{rapidjson.\+h}}. Some compilers can detect the setting, as in {\ttfamily \doxylink{perftest_8h}{perftest.\+h}}\+:
...
\doxysubsubsection*{Page boundary issue}
...
To fix this issue, currently the routine process bytes up to the next aligned address. After tha, use aligned read to perform SIMD processing. Also see \href{https://github.com/Tencent/rapidjson/issues/85}{\texttt{ \#85}}.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_LocalStreamCopy}{}\doxysubsection{\texorpdfstring{Local Stream Copy}{Local Stream Copy}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_LocalStreamCopy}
During optimization, it is found that some compilers cannot localize some member data access of streams into local variables or registers. Experimental results show that for some stream types, making a copy of the stream and used it in inner-\/loop can improve performance. For example, the actual (non-\/\+SIMD) implementation of {\ttfamily \doxylink{reader_8h_a60338858b2582eca23f3e509a2d82e0e}{Skip\+Whitespace()}} is implemented as\+:
...
\DoxyCodeLine{\ \ \ \ \mbox{\hyperlink{classinternal_1_1_stream_local_copy}{internal::StreamLocalCopy<InputStream>}}\ \mbox{\hyperlink{namespacefc_a79771837fffd80fcc4bc658ee2fc4b2b}{copy}}(is);}
\DoxyCodeLine{\ \ \ \ InputStream\&\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}(\mbox{\hyperlink{namespacefc_a79771837fffd80fcc4bc658ee2fc4b2b}{copy}}.s);}
\DoxyCodeLine{}
\DoxyCodeLine{\ \ \ \ \textcolor{keywordflow}{while}\ (\mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\ '}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)n'}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)r'}\ ||\ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Peek()\ ==\ \textcolor{charliteral}{'\(\backslash\)t'})}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \mbox{\hyperlink{yubihsm__pkcs11_8c_a506da415c9307c897ca9be93acfe1d0f}{s}}.Take();}
\DoxyCodeLine{\}}
...
Depending on the traits of stream, {\ttfamily Stream\+Local\+Copy} will make (or not make) a copy of the stream object, use it locally and copy the states of stream back to the original stream.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingDouble}{}\doxysubsection{\texorpdfstring{Parsing to Double}{Parsing to Double}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingDouble}
Parsing string into {\ttfamily double} is difficult. The standard library function {\ttfamily \doxylink{_parse_numbers_8cpp_a20d704fe51ff7001184f5bec5e2f7f8d}{strtod()}} can do the job but it is slow. By default, the parsers use normal precision setting. This has has maximum 3 \href{http://en.wikipedia.org/wiki/Unit_in_the_last_place}{\texttt{ ULP}} error and implemented in {\ttfamily \doxylink{namespaceinternal_a9e5fac4695310f09892fe52b1d24ab19}{internal\+::\+Strtod\+Normal\+Precision()}}.

When using {\ttfamily k\+Parse\+Full\+Precision\+Flag}, the parsers calls {\ttfamily \doxylink{namespaceinternal_a899ff5a0c5f9b203d720bb6f5631ed10}{internal\+::\+Strtod\+Full\+Precision()}} instead, and this function actually implemented 3 versions of conversion methods.
\begin{DoxyEnumerate}
...
If the first conversion methods fail, it will try the second, and so on.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_GenerationOptimization}{}\doxysection{\texorpdfstring{Generation Optimization}{Generation Optimization}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_GenerationOptimization}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_itoa}{}\doxysubsection{\texorpdfstring{Integer-\/to-\/\+String conversion}{Integer-\/to-\/\+String conversion}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_itoa}
...
Although SSE2 version is the fastest but the difference is minor by comparing to the first running-\/up {\ttfamily branchlut}. And {\ttfamily branchlut} is pure C++ implementation so we adopt {\ttfamily branchlut} in Rapid\+JSON.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_dtoa}{}\doxysubsection{\texorpdfstring{Double-\/to-\/\+String conversion}{Double-\/to-\/\+String conversion}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_dtoa}
Originally Rapid\+JSON uses {\ttfamily snprintf(..., ..., "{}\%g"{})} to achieve double-\/to-\/string conversion. This is not accurate as the default precision is 6. Later we also find that this is slow and there is an alternative.

Google\textquotesingle{}s V8 \href{https://github.com/floitsch/double-conversion}{\texttt{ double-\/conversion}} implemented a newer, fast algorithm called Grisu3 (Loitsch, Florian. "{}\+Printing floating-\/point numbers quickly and accurately with integers."{} \+ACM Sigplan Notices 45.6 (2010)\+: 233-\/243.).
...
The header-\/only conversion function has been evaluated in \href{https://github.com/miloyip/dtoa-benchmark}{\texttt{ dtoa-\/benchmark}}.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Parser}{}\doxysection{\texorpdfstring{Parser}{Parser}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Parser}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParser}{}\doxysubsection{\texorpdfstring{Iterative Parser}{Iterative Parser}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParser}
The iterative parser is a recursive descent LL(1) parser implemented in a non-\/recursive manner.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserGrammar}{}\doxysubsubsection{\texorpdfstring{Grammar}{Grammar}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserGrammar}
...
Note that left factoring is applied to non-\/terminals {\ttfamily values} and {\ttfamily members} to make the grammar be LL(1).\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserParsingTable}{}\doxysubsubsection{\texorpdfstring{Parsing Table}{Parsing Table}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserParsingTable}
...
\PBS\centering \doxylink{struct_s}{S}   &\PBS\centering \mbox{[} \{    \\\cline{1-2}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ NON-\/\+TERMINAL   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ FOLLOW    }\\\cline{1-2}
\endhead
\PBS\centering \doxylink{struct_s}{S}   &\PBS\centering \$    \\\cline{1-2}
\PBS\centering array   &\PBS\centering , \$ \} \mbox{]}    \\\cline{1-2}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ NON-\/\+TERMINAL   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \mbox{[}   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \{   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ ,   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \+:   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \mbox{]}   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \}   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ STRING   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ NUMBER   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ NULL   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ BOOLEAN    }\\\cline{1-11}
\endhead
\PBS\centering \doxylink{struct_s}{S}   &\PBS\centering array   &\PBS\centering object   &\PBS\centering &\PBS\centering &\PBS\centering &\PBS\centering &\PBS\centering &\PBS\centering &\PBS\centering &\PBS\centering \\\cline{1-11}
...
There is a great \href{http://hackingoff.com/compilers/predict-first-follow-set}{\texttt{ tool}} for above grammar analysis.\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserImplementation}{}\doxysubsubsection{\texorpdfstring{Implementation}{Implementation}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserImplementation}
