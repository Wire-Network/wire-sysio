\chapter{内部架构}
\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn}{}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn}\index{内部架构@{内部架构}}
本部分记录了一些设计和实现细节。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Architecture}{}\doxysection{\texorpdfstring{Architecture}{Architecture}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Architecture}
\doxysubsection*{SAX 和 DOM}

下面的 UML 图显示了 SAX 和 DOM 的基本关系。



关系的核心是 {\ttfamily Handler} 概念。在 SAX 一边，{\ttfamily Reader} 从流解析 JSON 并将事件发送到 {\ttfamily Handler}。{\ttfamily \doxylink{class_writer}{Writer}} 实现了 {\ttfamily Handler} 概念，用于处理相同的事件。在 DOM 一边，{\ttfamily Document} 实现了 {\ttfamily Handler} 概念，用于通过这些时间来构建 DOM。{\ttfamily Value} 支持了 {\ttfamily Value\+::\+Accept(Handler\&)} 函数，它可以将 DOM 转换为事件进行发送。

在这个设计，\+SAX 是不依赖于 DOM 的。甚至 {\ttfamily Reader} 和 {\ttfamily \doxylink{class_writer}{Writer}} 之间也没有依赖。这提供了连接事件发送器和处理器的灵活性。除此之外，{\ttfamily Value} 也是不依赖于 SAX 的。所以，除了将 DOM 序列化为 JSON 之外，用户也可以将其序列化为 XML，或者做任何其他事情。
...
{\ttfamily Value} （实际上被定义为 {\ttfamily \doxylink{class_generic_value}{Generic\+Value}\texorpdfstring{$<$}{<}\doxylink{struct_u_t_f8}{UTF8}\texorpdfstring{$<$}{<}\texorpdfstring{$>$}{>}\texorpdfstring{$>$}{>}}）是 DOM API 的核心。本部分描述了它的设计。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_DataLayout}{}\doxysubsection{\texorpdfstring{Data Layout}{Data Layout}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_DataLayout}
{\ttfamily Value} 是\href{http://en.wikipedia.org/wiki/Variant_type}{\texttt{ 可变类型}}。在 Rapid\+JSON 的上下文中，一个 {\ttfamily Value} 的实例可以包含6种 JSON 数据类型之一。通过使用 {\ttfamily union} ，这是可能实现的。每一个 {\ttfamily Value} 包含两个成员：{\ttfamily union \doxylink{struct_data}{Data} data\+\_\+} 和 {\ttfamily unsigned flags\+\_\+}。{\ttfamily flags\+\_\+} 表明了 JSON 类型，以及附加的信息。
...
\begin{longtabu}spread 0pt [c]{*{4}{|X[-1]}|}
\hline
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ \doxylink{struct_bool}{Bool}   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32位   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64位    }\\\cline{1-4}
\endfirsthead
\hline
\endfoot
\hline
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ String   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32位   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64位    }\\\cline{1-4}
\endhead
{\ttfamily Ch\texorpdfstring{$\ast$}{*} str}   &指向字符串的指针（可能拥有所有权）   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Object   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32位   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64位    }\\\cline{1-4}
\endhead
{\ttfamily Member\texorpdfstring{$\ast$}{*} members}   &指向成员数组的指针（拥有所有权）   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ Array   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 32位   }&\PBS\centering \cellcolor{\tableheadbgcolor}\textbf{ 64位    }\\\cline{1-4}
\endhead
{\ttfamily Value\texorpdfstring{$\ast$}{*} values}   &指向值数组的指针（拥有所有权）   &\PBS\centering 4   &\PBS\centering 8    \\\cline{1-4}
...
字符串有两个可选的标志。{\ttfamily k\+Copy\+Flag} 表明这个字符串拥有字符串拷贝的所有权。而 {\ttfamily k\+Inline\+Str\+Flag} 意味着使用了\doxysectlink{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}{短字符串优化}{2}。

数字更加复杂一些。对于普通的整数值，它可以包含 {\ttfamily k\+Int\+Flag}、{\ttfamily k\+Uint\+Flag}、 {\ttfamily k\+Int64\+Flag} 和/或 {\ttfamily k\+Uint64\+Flag}，这由整数的范围决定。带有小数或者超过64位所能表达的范围的整数的数字会被存储为带有 {\ttfamily k\+Double\+Flag} 的 {\ttfamily double}。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}{}\doxysubsection{\texorpdfstring{Short-\/\+String Optimization}{Short-\/\+String Optimization}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ShortString}
...
这个优化可以减少字符串拷贝内存占用。它也改善了缓存一致性，并进一步提高了运行时性能。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_InternalAllocator}{}\doxysection{\texorpdfstring{Allocator}{Allocator}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_InternalAllocator}
...
需要注意的是 {\ttfamily Malloc()} 和 {\ttfamily Realloc()} 是成员函数而 {\ttfamily Free()} 是静态成员函数。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_MemoryPoolAllocator}{}\doxysubsection{\texorpdfstring{Memory\+Pool\+Allocator}{Memory\+Pool\+Allocator}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_MemoryPoolAllocator}
{\ttfamily \doxylink{class_memory_pool_allocator}{Memory\+Pool\+Allocator}} 是 DOM 的默认内存分配器。它只申请内存而不释放内存。这对于构建 DOM 树非常合适。

在它的内部，它从基础的内存分配器申请内存块（默认为 {\ttfamily \doxylink{class_crt_allocator}{Crt\+Allocator}}）并将这些内存块存储为单向链表。当用户请求申请内存，它会遵循下列步骤来申请内存：


\begin{DoxyEnumerate}
\item 如果可用，使用用户提供的缓冲区。（见 \doxylink{dom_8md}{User Buffer section in DOM}）
...
你可以通过在包含 {\ttfamily \doxylink{rapidjson_8h}{rapidjson.\+h}} 之前定义 {\ttfamily RAPIDJSON\+\_\+\+SSE2} ， {\ttfamily RAPIDJSON\+\_\+\+SSE42} 或 {\ttfamily RAPIDJSON\+\_\+\+NEON} 来启用这个优化。一些编译器可以检测这个设置，如 {\ttfamily \doxylink{perftest_8h}{perftest.\+h}}：
...
为了修复这个问题，当前的代码会先按字节处理直到下一个对齐的地址。在这之后，使用对齐读取来进行 SIMD 处理。见 \href{https://github.com/Tencent/rapidjson/issues/85}{\texttt{ \#85}}。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_LocalStreamCopy}{}\doxysubsection{\texorpdfstring{Local Stream Copy}{Local Stream Copy}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_LocalStreamCopy}
在优化的过程中，我们发现一些编译器不能将访问流的一些成员数据放入局部变量或者寄存器中。测试结果显示，对于一些流类型，创建流的拷贝并将其用于内层循环中可以改善性能。例如，实际（非 SIMD）的 {\ttfamily \doxylink{reader_8h_a60338858b2582eca23f3e509a2d82e0e}{Skip\+Whitespace()}} 被实现为：
...
基于流的特征，{\ttfamily Stream\+Local\+Copy} 会创建（或不创建）流对象的拷贝，在局部使用它并将流的状态拷贝回原来的流。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingDouble}{}\doxysubsection{\texorpdfstring{Parsing to Double}{Parsing to Double}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_ParsingDouble}
将字符串解析为 {\ttfamily double} 并不简单。标准库函数 {\ttfamily \doxylink{_parse_numbers_8cpp_a20d704fe51ff7001184f5bec5e2f7f8d}{strtod()}} 可以胜任这项工作，但它比较缓慢。默认情况下，解析器使用默认的精度设置。这最多有 3\href{http://en.wikipedia.org/wiki/Unit_in_the_last_place}{\texttt{ ULP}} 的误差，并实现在 {\ttfamily \doxylink{namespaceinternal_a9e5fac4695310f09892fe52b1d24ab19}{internal\+::\+Strtod\+Normal\+Precision()}} 中。

当使用 {\ttfamily k\+Parse\+Full\+Precision\+Flag} 时，编译器会改为调用 {\ttfamily \doxylink{namespaceinternal_a899ff5a0c5f9b203d720bb6f5631ed10}{internal\+::\+Strtod\+Full\+Precision()}} ，这个函数会自动调用三个版本的转换。
\begin{DoxyEnumerate}
...
整数到字符串转换的朴素算法需要对每一个十进制位进行一次处罚。我们实现了若干版本并在 \href{https://github.com/miloyip/itoa-benchmark}{\texttt{ itoa-\/benchmark}} 中对它们进行了评估。

虽然 SSE2 版本是最快的，但它和第二快的 {\ttfamily branchlut} 差距不大。而且 {\ttfamily branchlut} 是纯\+C++实现，所以我们在 Rapid\+JSON 中使用了 {\ttfamily branchlut}。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_dtoa}{}\doxysubsection{\texorpdfstring{Double-\/to-\/\+String conversion}{Double-\/to-\/\+String conversion}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_dtoa}
原来 Rapid\+JSON 使用 {\ttfamily snprintf(..., ..., "{}\%g"{})} 来进行双精度浮点数到字符串的转换。这是不准确的，因为默认的精度是6。随后我们发现它很缓慢，而且有其它的替代品。

Google 的 V8 \href{https://github.com/floitsch/double-conversion}{\texttt{ double-\/conversion}} 实现了更新的、快速的被称为 Grisu3 的算法（\+Loitsch, Florian. "{}\+Printing floating-\/point numbers quickly and accurately with integers."{} \+ACM Sigplan Notices 45.6 (2010)\+: 233-\/243.）。

然而，这个实现不是仅头文件的，所以我们实现了一个仅头文件的 Grisu2 版本。这个算法保证了结果永远精确。而且在大多数情况下，它会生成最短的（可选）字符串表示。

这个仅头文件的转换函数在 \href{https://github.com/miloyip/dtoa-benchmark}{\texttt{ dtoa-\/benchmark}} 中进行评估。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Parser}{}\doxysection{\texorpdfstring{Parser}{Parser}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_Parser}
...
迭代解析器是一个以非递归方式实现的递归下降的 LL(1) 解析器。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserGrammar}{}\doxysubsubsection{\texorpdfstring{Grammar}{Grammar}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserGrammar}
...
注意到左因子被加入了非终结符的 {\ttfamily values} 和 {\ttfamily members} 来保证语法是 LL(1) 的。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserParsingTable}{}\doxysubsubsection{\texorpdfstring{Parsing Table}{Parsing Table}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserParsingTable}
...
对于上面的语法分析，这里有一个很棒的\href{http://hackingoff.com/compilers/predict-first-follow-set}{\texttt{ 工具}}。\hypertarget{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserImplementation}{}\doxysubsubsection{\texorpdfstring{Implementation}{Implementation}}\label{md_libraries_2fc_2include_2fc_2crypto_2webauthn__json_2doc_2internals_8zh-cn_IterativeParserImplementation}
