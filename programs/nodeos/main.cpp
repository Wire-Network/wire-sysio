...
{
   try {
      app().set_version(sysio::nodeos::config::version);
      app().set_version_string(sysio::version::version_client());
      app().set_full_version_string(sysio::version::version_full());

      auto root = fc::app_path();
      app().set_default_data_dir(root / "sysio" / nodeos::config::node_executable_name / "data" );
      app().set_default_config_dir(root / "sysio" / nodeos::config::node_executable_name / "config" );
