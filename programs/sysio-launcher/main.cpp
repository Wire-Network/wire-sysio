...
using namespace sysio::launcher::config;
...
struct server_name_def {
  ...
  string sysio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), sysio_home(), instances(1) {}
};
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodeop_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodeop_installation_paths);
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodeop_args[num] = specific_args[i];
    }
  }
  ...
  if (per_host == 0) {
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->sysio_home =
          (local_id.contains (lhost->host_name) || servers.default_sysio_home.empty()) ?
          erd : servers.default_sysio_home;
        host_ndx++;
      } // ph_count == 0
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->sysio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->sysio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->sysio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->sysio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->sysio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (node.producers.size()) {
    ...
    cfg << "plugin = sysio::producer_plugin\n";
  }
  cfg << "plugin = sysio::net_plugin\n";
  cfg << "plugin = sysio::chain_api_plugin\n";
  cfg.close();
}
...
launcher_def::prep_remote_config_dir (eosd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->sysio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->sysio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->sysio_home;

  cmd = "cd " + host->sysio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->sysio_home
...
launcher_def::launch (eosd_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodeop_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodeop_installation_paths.count(node_num)) {
        install_path = specific_nodeop_installation_paths[node_num] + "/";
     }
  }
  string eosdcmd = install_path + "programs/nodeop/" + string(node_executable_name) + " ";
  ...
  if (instance.name != "bios" && !specific_nodeop_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodeop_args.count(node_num)) {
        eosdcmd += specific_nodeop_args[node_num] + " ";
     }
  }
  ...
  if (eosdcmd.find("sysio::history_api_plugin") != string::npos && eosdcmd.find("sysio::trace_api_plugin") != string::npos){
    // remove trace_api_plugin from old version nodes in multiversion test
    eosdcmd = find_and_remove_arg(eosdcmd, "--plugin sysio::trace_api_plugin");
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->sysio_home + "; nohup " + eosdcmd + " > "
    ...
    string cmd = "cd " + host->sysio_home + "; kill -15 $(cat " + pidf.string() + ")";
...
launcher_def::do_command(const host_def& host, const string& name,
   ...
   if (!host.is_local()) {
      string rcmd = "cd " + host.sysio_home + "; ";
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/sysio-tn_bounce.sh " + eosd_extra_args;
      if (node_num != "bios" && !specific_nodeop_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodeop_args.count(node_num_i)) {
            cmd += " " + specific_nodeop_args[node_num_i];
         }
      }
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the sysio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the sysio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the sysio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << sysio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(sysio_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (sysio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_sysio_home) (ssh) )
