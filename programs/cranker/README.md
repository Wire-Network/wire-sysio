...
`cranker` is an application that uses the `outpost_ethereum_client_plugin` and `cron_plugin` to monitor and update an Ethereum network.

## Usage

To run `cranker`, you need to provide at least one Ethereum signature provider, one Ethereum outpost client, and one Ethereum ABI file.
