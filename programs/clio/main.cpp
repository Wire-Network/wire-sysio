...
void print_action( const fc::variant& at ) {
   ...
   if( code == "sysio" && func == "setcode" )
      args = args.substr(40)+"...";
   if( name(code) == sysio::chain::config::system_account_name && func == "setabi" )
      args = args.substr(40)+"...";
   */
...
chain::action create_newaccount(const name& creator, const name& newaccount, authority owner, authority active) {
   return action {
      get_account_permissions(tx_permission, {creator,sysio::chain::config::active_name}),
...
chain::action create_open(const string& contract, const name& owner, symbol sym, const name& ram_payer) {
   auto open_ = fc::mutable_variant_object
      ...
      ("ram_payer", ram_payer);
    return action {
      get_account_permissions(tx_permission, {ram_payer, sysio::chain::config::active_name}),
...
chain::action create_transfer(const string& contract, const name& sender, const name& recipient, asset amount, const string& memo ) {
   ...
   return action {
      get_account_permissions(tx_permission, {sender,sysio::chain::config::active_name}),
...
chain::action create_setabi(const name& account, const bytes& abi) {
   return action {
      get_account_permissions(tx_permission, {account,sysio::chain::config::active_name}),
      setabi{
         .account   = account,
         .abi       = abi
      }
...
chain::action create_updateauth(const name& account, const name& permission, const name& parent, const authority& auth) {
   return action { get_account_permissions(tx_permission, {account,sysio::chain::config::active_name}),
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_parent ) {
               ...
               if ( itr != res.permissions.end() ) {
                  ...
                  parent = sysio::chain::config::active_name.to_string();
               }
            }
...
struct register_producer_subcommand {
   ...
   register_producer_subcommand(CLI::App* actionRoot) {
      ...
      register_producer->callback([this] {
         ...
         send_actions({create_action(accountPermissions, sysio::chain::config::system_account_name, "regproducer"_n, regprod_var)}, signing_keys_opt.get_keys());
      });
   }
};
...
struct unregister_producer_subcommand {
   ...
   unregister_producer_subcommand(CLI::App* actionRoot) {
      ...
      unregister_producer->callback([this] {
         ...
         auto accountPermissions = get_account_permissions(tx_permission, {name(producer_str), sysio::chain::config::active_name});
         send_actions({create_action(accountPermissions, sysio::chain::config::system_account_name, "unregprod"_n, act_payload)}, signing_keys_opt.get_keys());
      });
   }
};
...
struct claimrewards_subcommand {
   ...
   claimrewards_subcommand(CLI::App* actionRoot) {
      ...
      claim_rewards->callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ("owner", owner);
         auto accountPermissions = get_account_permissions(tx_permission, {name(owner), sysio::chain::config::active_name});
         send_actions({create_action(accountPermissions, sysio::chain::config::system_account_name, "claimrewards"_n, act_payload)}, signing_keys_opt.get_keys());
      });
   }
};
...
int main( int argc, char** argv ) {
   ...
   string type;
   string abi_file;
   auto pack_hex = convert_cmd->add_subcommand("pack_hex", localized("From JSON to packed HEX form"));
   ...
   // unpack hex
   string packed_hex;
   auto unpack_hex = convert_cmd->add_subcommand("unpack_hex", localized("From packed HEX to JSON form"));
   ...
   // get finalizer info
   get_cmd->add_subcommand("finalizer_info", localized("Get current finalizer information"))->callback([] {
 ...
 //                                ->check(CLI::ExistingFile);
   ...
   transfer_cmd->add_option("--contract,-c", con, localized("The contract that controls the token, defaults to sysio.token for asset amounts"));
   ...
   action_cmd->add_flag("--dry-run", tx_dry_run, localized("Specify an action is dry-run"));
   ...
   propose_action_cmd->callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!proposer.empty()) {
            accountPermissions = vector<permission_level>{{name(proposer), sysio::chain::config::active_name}};
         } else {
   ...
   propose_trx_cmd->callback([&] {
      ...
      if (account_permissions.empty()) {
         if (!proposer.empty()) {
            account_permissions = vector<permission_level>{{name(proposer), sysio::chain::config::active_name}};
         } else {
            SYS_THROW(missing_auth_exception, "Authority is not provided (either by multisig parameter <proposer> or -p)");
         }
      }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      auto accountPermissions = get_account_permissions(tx_permission, {name(proposer), sysio::chain::config::active_name});
   ...
   invalidate->callback([&] {
      ...
      auto accountPermissions = get_account_permissions(tx_permission, {name(invalidator), sysio::chain::config::active_name});
   ...
   cancel->callback([&]() {
      ...
      if (accountPermissions.empty()) {
         if (!canceler.empty()) {
            accountPermissions = vector<permission_level>{{name(canceler), sysio::chain::config::active_name}};
         } else {
   ...
   exec->callback([&] {
      ...
      if (accountPermissions.empty()) {
         if (!executer.empty()) {
            accountPermissions = vector<permission_level>{{name(executer), sysio::chain::config::active_name}};
         } else {
   ...
   wrap_exec->callback([&] {
      ...
      auto accountPermissions = get_account_permissions(tx_permission);
      if( accountPermissions.empty() ) {
         accountPermissions = vector<permission_level>{{name(executer), sysio::chain::config::active_name}, {name(wrap_con), sysio::chain::config::active_name}};
      }
