...
#include <sysio/state_history/counter.hpp>
...
private:
 ...
 public:
   ...
   state_history_log(const std::filesystem::path& log_dir_and_stem,
     ...
     index(std::filesystem::path(log_dir_and_stem).replace_extension("index")) {
      SYS_ASSERT(!!non_local_get_block_id, chain::plugin_exception, "misuse of get_block_id");
   ...
   std::optional<ship_log_entry> get_entry(uint32_t block_num) {
      ...
      // 1) up through and including sysio 2.0 would add an uint32_t indicating compressed message size
 ...
 private:
   ...
   void vacuum() {
      ...
      log_header first_header = log.unpack_from<decltype(first_header)>(0);
      SYS_ASSERT(is_ship_log_pruned(first_header.magic), chain::plugin_exception, "vacuum can only be performed on pruned logs");
...
FC_REFLECT(sysio::state_history::log_header, (magic)(block_id)(payload_size))
FC_REFLECT_DERIVED(sysio::state_history::log_header_with_sizes, (sysio::state_history::log_header), (compressed_size)(uncompressed_size));
