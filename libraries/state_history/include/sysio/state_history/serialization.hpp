...
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<sysio::chain::kv_object>& obj) {
   ...
   if (obj.obj.key_format == 1 && obj.obj.key_size == 24) {
      ...
      // Non-standard key: emit code + raw key + value
      fc::raw::pack(ds, as_type<uint64_t>(obj.obj.code.to_uint64_t()));
      fc::raw::pack(ds, as_type<uint64_t>(0));                           // scope = 0
      fc::raw::pack(ds, as_type<uint64_t>(0));                           // table = 0
      fc::raw::pack(ds, as_type<uint64_t>(0));                           // primary_key = 0
      fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t())); // payer
      // Pack key + value together as the value field
      std::vector<char> combined;
      combined.reserve(2 + obj.obj.key_size + obj.obj.value.size());
      uint16_t ks = obj.obj.key_size;
      combined.insert(combined.end(), reinterpret_cast<const char*>(&ks), reinterpret_cast<const char*>(&ks) + 2);
      combined.insert(combined.end(), obj.obj.key_data(), obj.obj.key_data() + obj.obj.key_size);
      combined.insert(combined.end(), obj.obj.value.data(), obj.obj.value.data() + obj.obj.value.size());
      history_pack_big_bytes(ds, combined);
   }
   return ds;
}
