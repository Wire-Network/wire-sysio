...
void pack_deltas(boost::iostreams::filtering_ostreambuf& obuf, const chainbase::database& db, bool full_snapshot) {
   ...
   auto is_standard_kv = [](const chain::kv_object& o) { return o.key_format == chain::config::kv_format_standard && o.key.size() == chain::kv_key_size; };
   auto is_raw_kv      = [](const chain::kv_object& o) { return o.key_format != chain::config::kv_format_standard || o.key.size() != chain::kv_key_size; };
