    ...
    namespace testing {
        ...
        void base_tester::init(const setup_policy policy, db_read_mode read_mode,
            ...
            auto def_conf = default_config(tempdir, genesis_max_inline_action_size,
                                           config_max_nonprivileged_inline_action_size);
            ...
            if (!skip_pending_trxs) {
                for (auto itr = unapplied_transactions.begin(); itr != unapplied_transactions.end();) {
                    auto trace = control->push_transaction(itr->trx_meta, fc::time_point::maximum(),
                                                           fc::microseconds::maximum(), DEFAULT_BILLED_CPU_TIME_US,
                                                           true, 0);
                ...
                while ((scheduled_trxs = get_scheduled_transactions()).size() > 0) {
                    for (const auto &trx: scheduled_trxs) {
                        auto trace = control->push_scheduled_transaction(
                            trx, fc::time_point::maximum(), fc::microseconds::maximum(), DEFAULT_BILLED_CPU_TIME_US,
                            true);
                        traces.emplace_back(trace);
                        if (!no_throw && trace->except) {
                            // this always throws an fc::exception, since the original exception is copied into an fc::exception
        ...
        transaction_trace_ptr base_tester::push_transaction(packed_transaction &trx,
            ...
            try {
                ...
                auto fut = transaction_metadata::start_recover_keys(ptrx, control->get_thread_pool(),
                                                                    control->get_chain_id(), time_limit,
                                                                    transaction_metadata::trx_type::input);
                auto r = control->push_transaction(fut.get(), deadline, fc::microseconds::maximum(), billed_cpu_time_us,
                                                   billed_cpu_time_us > 0, 0);
        ...
        transaction_trace_ptr base_tester::transfer(account_name from, account_name to, string amount, string memo,
                                                    account_name currency) {
            return transfer(from, to, asset::from_string(amount), memo, currency);
        }


        transaction_trace_ptr base_tester::transfer(account_name from, account_name to, asset amount, string memo,
                                                    account_name currency) {
            fc::variant pretty_trx = fc::mutable_variant_object()
            ("actions", fc::variants({
                 fc::mutable_variant_object()
        ...
        void base_tester::link_authority(account_name account, account_name code, permission_name req,
                                         action_name type) {
            signed_transaction trx;
        ...
        void base_tester::set_code(account_name account, const vector<uint8_t> wasm,
        ...
        asset base_tester::get_currency_balance(const account_name &code,
            ...
            const auto *tbl = db.template find<table_id_object, by_code_scope_table>(
                boost::make_tuple(code, account, "accounts"_n));
            share_type result = 0;
            ...
            auto sync_dbs = [](base_tester &a, base_tester &b) {
                for (uint32_t i = 1; i <= a.control->head_block_num(); ++i) {
                    ...
                    if (block) {
                        ...
                        b.control->push_block(bsf.get(), forked_branch_callback{}, trx_meta_cache_lookup{});
                        //, sysio::chain::validation_steps::created_block);
                    }
                }
            };
        ...
        void base_tester::preactivate_all_builtin_protocol_features() {
            ...
            const auto current_block_time = (control->is_building_block()
                                                 ? control->pending_block_time()
                                                 : control->head_block_time() + fc::milliseconds(
            ...
            std::function<void(const digest_type &)> add_digests =
                    [&pfm, &pfs, current_block_num, current_block_time, &preactivation_set, &preactivations, &
                        add_digests]
