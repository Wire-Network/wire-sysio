...
namespace sysio::testing {
  ...
  void base_tester::set_transaction_headers( transaction& trx, uint32_t expiration ) const {
   ...
   typename base_tester::action_result base_tester::push_contract_paid_action(action&& act, uint64_t authorizer) {
      signed_transaction trx;
      if (authorizer) {
         act.authorization = vector<permission_level>{
            {account_name(authorizer), sysio::chain::config::active_name}};
   ...
   asset base_tester::get_currency_balance( const account_name& code,
      ...
      auto kv_itr = kv_idx.find( boost::make_tuple( code, std::string_view(key.data, 24) ) );
