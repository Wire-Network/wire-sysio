...
namespace std {
   inline std::ostream& operator<<(std::ostream& os, const sysio::chain::finalizer_safety_information& fsi) {
      os << "fsi(" << fsi.last_vote << ", " << fsi.lock << ", " << fsi.other_branch_latest_time << ")";
      return os;
   }

   inline std::ostream& operator<<(std::ostream& os, const sysio::chain::finalizer::vote_result& vr) {
      os << "vote_result(\"";
      using vote_decision = sysio::chain::finalizer::vote_decision;
      switch(vr.decision) {
...
FC_REFLECT(sysio::chain::finalizer_safety_information, (last_vote)(lock)(other_branch_latest_time))
FC_REFLECT_ENUM(sysio::chain::finalizer::vote_decision, (strong_vote)(weak_vote)(no_vote))
