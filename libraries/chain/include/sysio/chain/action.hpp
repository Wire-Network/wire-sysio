...
namespace sysio { namespace chain {
    ...
    *  In the context of sysio, every action is dispatched to the handler defined
   ...
   struct action : public action_base {
      ...
      template<typename T>
      T data_as()const {
         SYS_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
...
FC_REFLECT( sysio::chain::permission_level, (actor)(permission) )
FC_REFLECT( sysio::chain::action_base, (account)(name)(authorization) )
FC_REFLECT_DERIVED( sysio::chain::action, (sysio::chain::action_base), (data) )
