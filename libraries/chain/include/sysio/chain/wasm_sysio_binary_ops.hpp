...
namespace sysio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return sysio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       sysio::chain::wasm_ops::to_string(f); }
};
template <>
...
struct SYSIO_OperatorDecoderStream
{
   ...
   instr* decodeOp() {
      SYS_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      IR::Opcode opcode;
...
template <class Op_Types>
const std::vector<instr*>* SYSIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace sysio, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), sysio::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( sysio::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( sysio::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( sysio::chain::wasm_ops::memoryoptype, (end) )
