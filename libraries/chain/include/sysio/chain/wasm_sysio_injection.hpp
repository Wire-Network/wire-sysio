...
namespace sysio { namespace chain { namespace wasm_injections {
   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, sysio_injected_module_name, std::move(func_name)} };
   ...
   struct f32_promote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f64, ValueType::f32>( *(arg.module), u8"_sysio_f32_promote", idx );
   ...
   struct f64_demote_injector {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         int32_t idx;
         injector_utils::add_import<ResultType::f32, ValueType::f64>( *(arg.module), u8"_sysio_f64_demote", idx );
         wasm_ops::op_types<>::call_t f32promote;
         f32promote.field = idx;
         f32promote.pack(arg.new_code);
      }
...
}}} // namespace wasm_constraints, chain, sysio
