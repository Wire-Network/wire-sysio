...
namespace sysio {
  namespace chain {
    ...
    class named_thread_pool {
      const std::string _thread_pool_prefix_label;
      ...
      public:
        ...
        explicit named_thread_pool(const std::optional<std::string>& prefix_label_opt = std::nullopt) : _thread_pool_prefix_label(
          prefix_label_opt.value_or(fc::pretty_type<NamePrefixTag>{}.name())
        ) {
          ...
          try {
            for (size_t i = 0; i < num_threads; ++i) {
              _thread_pool.emplace_back(
                std::thread(
                  &named_thread_pool::run_thread,
                  this,
                  i,
                  on_except,
                  init,
                  std::ref(start_complete),
                  std::ref(threads_remaining),
                  std::ref(pending_exception),
                  std::ref(pending_exception_mutex)
                )
              );
            }
          } catch (...) {
      ...
      private:
        ...
        void run_thread(
          size_t i,
          const on_except_t& on_except,
          const init_t& init,
          std::promise<void>& start_complete,
          std::atomic<uint32_t>& threads_remaining,
          std::exception_ptr& pending_exception,
          std::mutex& pending_exception_mutex
        ) {
          ...
          auto decrement_remaining = [&]() {
            if (!--threads_remaining) {
              if (pending_exception) start_complete.set_exception(pending_exception);
              else start_complete.set_value();
            }
          };
