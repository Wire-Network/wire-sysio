...
namespace sysio { namespace chain {
   ...
   namespace config {
      ...
      struct billable_size<permission_link_object> {
         ...
         static_assert(sizeof(permission_link_object) <= value, "billable_size<permission_link_object> must be >= sizeof(permission_link_object)");
      };
   }
} } // sysio::chain

CHAINBASE_SET_INDEX_TYPE(sysio::chain::permission_link_object, sysio::chain::permission_link_index)

FC_REFLECT(sysio::chain::permission_link_object, (account)(code)(message_type)(required_permission))
