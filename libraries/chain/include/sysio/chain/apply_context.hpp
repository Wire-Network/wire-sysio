...
class apply_context {
   ...
   public:
      ...
      int32_t  kv_get(name code, const char* key, uint32_t key_size, char* value, uint32_t value_size);
      int64_t  kv_erase(const char* key, uint32_t key_size);
      int32_t  kv_contains(name code, const char* key, uint32_t key_size);

      // Primary KV iterators
      uint32_t kv_it_create(name code, const char* prefix, uint32_t prefix_size);
      ...
      void     kv_idx_store(name table, uint8_t index_id, const char* sec_key, uint32_t sec_key_size,
                            const char* pri_key, uint32_t pri_key_size);
      void     kv_idx_remove(name table, uint8_t index_id, const char* sec_key, uint32_t sec_key_size,
                             const char* pri_key, uint32_t pri_key_size);
      void     kv_idx_update(name table, uint8_t index_id,
                             const char* old_sec_key, uint32_t old_sec_key_size,
                             const char* new_sec_key, uint32_t new_sec_key_size,
                             const char* pri_key, uint32_t pri_key_size);
      uint32_t kv_idx_find_secondary(name code, name table, uint8_t index_id,
                                     const char* sec_key, uint32_t sec_key_size);
      uint32_t kv_idx_lower_bound(name code, name table, uint8_t index_id,
                                  const char* sec_key, uint32_t sec_key_size);
