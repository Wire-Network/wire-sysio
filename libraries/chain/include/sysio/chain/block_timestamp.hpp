   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            SYS_ASSERT( std::numeric_limits<uint32_t>::max() - slot >= 1, fc::overflow_exception, "block timestamp overflow" );
...
namespace std {
   inline std::ostream& operator<<(std::ostream& os, const sysio::chain::block_timestamp_type& t) {
      os << "tstamp(" << t.slot << ")";
      return os;
   }
}
