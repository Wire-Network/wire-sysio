...
struct log_catalog {
   ...
   std::vector<block_pos_size_t> get_block_positions_and_sizes(uint32_t block_num, uint32_t count, uint32_t& blocks_read) {
      ...
      const uint32_t file_last  = log_data.last_block_num();
      const uint32_t file_first = log_data.first_block_num();
      const uint32_t available  = file_last - block_num + 1;
      const uint32_t n          = std::min(count, available);
      ...
      const bool need_file_size_for_last = (block_num + n - 1 == file_last);
      const uint32_t positions_to_read   = need_file_size_for_last ? n : n + 1;
      const uint64_t index_offset        = (block_num - file_first) * pos_size;
      ...
      if (need_file_size_for_last)
         positions.push_back(log_data.size());
