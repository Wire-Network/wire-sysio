...
struct subjective_compile_limits {
   ...
   // nodeop enforces the limits by the default values unless account is whitelisted.
   // libtester disables the limits in all tests, except enforces the limits
   // in the tests in unittests/sysvmoc_limits_tests.cpp.
...
inline DS& operator>>(DS& ds, sysio::chain::sysvmoc::subjective_compile_limits& cl) {
...
template <typename DS>
inline DS& operator<<(DS& ds, const sysio::chain::sysvmoc::subjective_compile_limits& cl) {
...
FC_REFLECT(sysio::chain::sysvmoc::config, (cache_size)(threads)(non_whitelisted_limits))