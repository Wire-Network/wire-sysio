...
struct config {
   ...
   // nodeop enforces the limits by the default values.
...
inline DS& operator>>(DS& ds, sysio::chain::sysvmoc::config& cfg) {
...
template <typename DS>
inline DS& operator<<(DS& ds, const sysio::chain::sysvmoc::config& cfg) {
   fc::raw::pack(ds, cfg.cache_size);
   fc::raw::pack(ds, cfg.threads);
