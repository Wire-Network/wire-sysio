...
class code_cache_async : public code_cache_base {
   public:
      ...
      code_cache_async(const std::filesystem::path& data_dir, const sysvmoc::config& sysvmoc_config,
   ...
   private:
      ...
      void write_message(const digest_type& code_id, const sysvmoc_message& message, std::span<wrapped_fd> fds);
