      ...
      public:
        ...
        */
         ...
         */
         ...
         int32_t  kv_get(uint64_t code, legacy_span<const char> key, legacy_span<char> value);
         int64_t  kv_erase(legacy_span<const char> key);
         int32_t  kv_contains(uint64_t code, legacy_span<const char> key);

         // KV database api - primary iterators
         uint32_t kv_it_create(uint64_t code, legacy_span<const char> prefix);
         ...
         void     kv_idx_store(uint64_t table, uint32_t index_id, legacy_span<const char> sec_key, legacy_span<const char> pri_key);
         void     kv_idx_remove(uint64_t table, uint32_t index_id, legacy_span<const char> sec_key, legacy_span<const char> pri_key);
         void     kv_idx_update(uint64_t table, uint32_t index_id, legacy_span<const char> old_sec_key,
                                legacy_span<const char> new_sec_key, legacy_span<const char> pri_key);
         uint32_t kv_idx_find_secondary(uint64_t code, uint64_t table, uint32_t index_id, legacy_span<const char> sec_key);
         uint32_t kv_idx_lower_bound(uint64_t code, uint64_t table, uint32_t index_id, legacy_span<const char> sec_key);
