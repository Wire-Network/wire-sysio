...
namespace sysio { namespace chain { namespace webassembly {
   ...
   int64_t interface::kv_set(uint32_t table_id, uint64_t payer, span<const char> key, span<const char> value) {
      return context.kv_set(checked_table_id(table_id), payer, key.data(), key.size(), value.data(), value.size());
   }

   int32_t interface::kv_get(uint32_t table_id, uint64_t code, span<const char> key, span<char> value) {
      return context.kv_get(checked_table_id(table_id), name(code), key.data(), key.size(), value.data(), value.size());
   }

   int64_t interface::kv_erase(uint32_t table_id, span<const char> key) {
      return context.kv_erase(checked_table_id(table_id), key.data(), key.size());
   }

   int32_t interface::kv_contains(uint32_t table_id, uint64_t code, span<const char> key) {
      return context.kv_contains(checked_table_id(table_id), name(code), key.data(), key.size());
   }

   // KV primary iterators
   uint32_t interface::kv_it_create(uint32_t table_id, uint64_t code, span<const char> prefix) {
   ...
   int32_t interface::kv_it_lower_bound(uint32_t handle, span<const char> key) {
   ...
   void interface::kv_idx_store(uint64_t payer, uint32_t table_id, span<const char> pri_key, span<const char> sec_key) {
      context.kv_idx_store(payer, checked_table_id(table_id),
                           pri_key.data(), pri_key.size(), sec_key.data(), sec_key.size());
   }

   void interface::kv_idx_remove(uint32_t table_id, span<const char> pri_key, span<const char> sec_key) {
      context.kv_idx_remove(checked_table_id(table_id),
                            pri_key.data(), pri_key.size(), sec_key.data(), sec_key.size());
   }

   void interface::kv_idx_update(uint64_t payer, uint32_t table_id, span<const char> pri_key,
   ...
   int32_t interface::kv_idx_find_secondary(uint64_t code, uint32_t table_id, span<const char> sec_key) {
      return context.kv_idx_find_secondary(name(code), checked_table_id(table_id),
                                           sec_key.data(), sec_key.size());
   }

   int32_t interface::kv_idx_lower_bound(uint64_t code, uint32_t table_id, span<const char> sec_key) {
