...
ethereum_client::ethereum_client(const signature_provider_ptr& sig_provider, const std::variant<std::string, fc::url>& url_source, std::optional<fc::uint256> chain_id)
...
fc::uint256 ethereum_client::get_chain_id() {
   ...
   _chain_id = parse_uint256(execute("eth_chainId", params));
...
fc::uint256 ethereum_client::get_network_version() {
   ...
   return parse_uint256(execute("net_version", params));
}
