...
namespace fc { namespace crypto {
   ...
   signature signature::from_string(const std::string& str, sig_type type) {
      ...
      case sig_type::em: {
         ...
         return from_native_string_to_signature<chain_key_type_t::ethereum>(sig);
      }
      case sig_type::ed: {
         auto [base_prefix, type_prefix, data_str] = parse_base_prefixes(str);
         const auto& sig = base_prefix.empty() ? str : data_str;
         FC_ASSERT(type_prefix.empty() || type_prefix == sig_prefix(sig_type::ed), "Invalid signature prefixes: {}", str);
         return from_native_string_to_signature<chain_key_type_t::solana>(sig);
      }
      case sig_type::unknown: {
         ...
         if (type_prefix == sig_prefix(sig_type::em)) {
            return from_native_string_to_signature<chain_key_type_t::ethereum>(data_str);
         } else if (type_prefix == sig_prefix(sig_type::ed)) {
            return from_native_string_to_signature<chain_key_type_t::solana>(data_str);
         }
