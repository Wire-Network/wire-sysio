...
constexpr uint32_t internal_server_error_status =
   static_cast<uint32_t>(boost::beast::http::status::internal_server_error);
constexpr uint32_t not_found_status = static_cast<uint32_t>(boost::beast::http::status::not_found);
...
public:
   explicit transport_impl(transport_options options, detail::resolver_start_fn resolver_start = {})
      : async_client(io.get_executor(), std::move(options), std::move(resolver_start))
      , poll_timer(io) {}
   ...
   class use_guard {
   public:
      use_guard(transport_impl& owner, request_options& options)
         : _owner(owner)
         , _lock(owner.use_mutex, std::defer_lock)
         , _previous(active_transport) {
         if (active_transport == &_owner) {
            FC_THROW("Outbound HTTP synchronous transport cannot be re-entered");
         }
         ...
         if (options.timeouts.total) {
            ...
            configured_deadline.safe_add(*options.timeouts.total);
            deadline = configured_deadline;
         }
         if (options.timeouts.inherit_task_deadline) {
            if (const auto task_deadline = fc::task::current_deadline();
                task_deadline && (!deadline || *task_deadline < *deadline)) {
         ...
         if (!deadline && !options.cancel_check) {
            if (!_lock.try_lock()) {
               FC_THROW("Outbound HTTP synchronous transport is busy and the request has no queue deadline");
            }
         } else {
            while (!_lock.owns_lock()) {
               bool cancelled = false;
               try {
                  cancelled = options.cancel_check && options.cancel_check();
               } catch (...) {
                  cancelled = true;
               }
               if (cancelled) {
               ...
               if (deadline) {
                  const auto remaining = *deadline - time_point::now();
                  if (remaining.count() <= 0) {
                     FC_THROW_EXCEPTION(
                        fc::timeout_exception,
                        "Outbound HTTP total request deadline expired while waiting for the synchronous transport");
                  }
                  wait_for = std::min(wait_for, std::chrono::duration_cast<std::chrono::milliseconds>(
         ...
         if (options.timeouts.total) {
            const auto elapsed = time_point::now() - started;
            if (elapsed >= *options.timeouts.total) {
               FC_THROW_EXCEPTION(
                  fc::timeout_exception,
                  "Outbound HTTP total request deadline expired while waiting for the synchronous transport");
            }
            options.timeouts.total = *options.timeouts.total - elapsed;
         }
      ...
      ~use_guard() { active_transport = _previous; }
   ...
   response perform(const request& req, const request_options& options) {
      ...
      return run<response>(
         [&](asio::cancellation_slot slot) { return async_client.async_request(req, std::move(async_options), slot); },
   ...
   void perform_to_file(const request& req, const request_options& options, const std::filesystem::path& output,
      ...
      run_void(
         [&](asio::cancellation_slot slot) {
            return async_download_atomic(async_client, req, std::move(async_options), output,
                                         download_options{
                                            .status_callback = status_callback,
                                            .space_available_provider = space_available_provider,
                                         },
                                         slot);
         },
         options.cancel_check);
...
private:
   ...
   void arm_cancel_poll(const std::function<bool()>& cancel_check, asio::cancellation_signal& cancellation,
      ...
      poll_timer.async_wait([this, &cancel_check, &cancellation, &complete](const boost::system::error_code& error) {
         if (error || complete)
            return;
         bool cancel = false;
         try {
   ...
   Result run(Factory&& factory, const std::function<bool()>& cancel_check) {
      asio::cancellation_signal cancellation;
      std::optional<Result> result;
      std::exception_ptr failure;
      bool complete = false;
      asio::co_spawn(io, factory(cancellation.slot()),
                     [&](std::exception_ptr operation_failure, Result operation_result) {
   ...
   void run_void(Factory&& factory, const std::function<bool()>& cancel_check) {
      asio::cancellation_signal cancellation;
      std::exception_ptr failure;
      bool complete = false;
      asio::co_spawn(io, factory(cancellation.slot()), [&](std::exception_ptr operation_failure) {
         failure = std::move(operation_failure);
         complete = true;
         poll_timer.cancel();
      });
...
transport::transport(transport_options options, detail::resolver_start_fn resolver_start)
   : _impl(std::make_unique<transport_impl>(std::move(options), std::move(resolver_start))) {}
...
void transport::perform_to_file(const request& req, const request_options& options, const std::filesystem::path& output,
                                const std::function<void(const http_file_download_status&)>& status_callback,
                                const std::function<uint64_t(const std::filesystem::path&)>& space_available_provider) {
   _impl->perform_to_file(req, options, output, status_callback, space_available_provider);
}
...
http::timeout_options legacy_timeouts(time_point deadline) {
   ...
   if (deadline < time_point::maximum()) {
      ...
      FC_ASSERT(now < deadline, "HTTP request deadline already expired");
...
http_client::http_client(http::transport_options options)
   : _transport(std::make_unique<http::transport>(std::move(options))) {}
...
void http_client::post_to_file(const url& dest, const variant& payload, const std::filesystem::path& output,
   ...
   http::request req{
      ...
      .target = dest,
      .body = json::to_string(payload, time_point::maximum()),
   ...
   http::request_options policy{
      ...
      .retry =
         http::retry_options{
                             .max_attempts = options.retry_failed_reused_connection ? 2U : 1U,
                             .allow_retry = options.retry_failed_reused_connection
                              ? std::function<bool(const http::retry_context&)>{[](const http::retry_context& context) {
                                   return context.reused_connection;
                                }}
                              : std::function<bool(const http::retry_context&)>{},
                             },
      .idempotent = options.retry_failed_reused_connection,
      .cancel_check = _cancel_check,
   };
   _transport->perform_to_file(req, policy, output, options.status_callback, _space_available_provider);
}
...
variant http_client::post_sync(const url& dest, const variant& payload, const time_point& deadline) {
   ...
   http::request req{
      .method = http::request_method::post,
      .target = dest,
      .body = json::to_string(payload, serialization_deadline),
      .content_type = "application/json",
      .user_agent = "wire-libfc-http",
   };
   http::request_options policy{
   ...
   if (response.status == http::internal_server_error_status) {
      exception_ptr remote;
      try {
         const auto error = result.get_object()["error"].get_object();
         remote =
            std::make_shared<exception>(error["code"].as_int64(), error["name"].as_string(), error["what"].as_string());
         if (error.contains("details")) {
            for (const auto& detail : error["details"].get_array()) {
               remote->append_log(FC_LOG_MESSAGE(error, "{}", detail.get_object()["message"].as_string()));
            }
         }
      } catch (...) {}
      if (remote)
   ...
   if (boost::beast::http::to_status_class(response.status) != boost::beast::http::status_class::successful) {
