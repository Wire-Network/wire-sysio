...
public:
   ...
   static void arm_operation_deadline(Stream& stream, const std::optional<operation_deadline>& deadline) {
      if (!deadline) {
   ...
   static void throw_if_operation_failed(const error_code& error, const std::optional<operation_deadline>& deadline,
   ...
   asio::awaitable<size_t> read_body(const std::shared_ptr<connection_state>& connection, Stream& stream,
      ...
      active_cancel_guard cancel_guard(control, [connection] { connection->cancel(); });
      error_code error;
      (void)co_await beast_http::async_read_some(stream, buffer, parser,
                                                 asio::redirect_error(asio::use_awaitable, error));
      throw_if_operation_failed(error, deadline, control);
   ...
   asio::awaitable<size_t> read_some(asio::mutable_buffer output) {
      ...
      try {
         const auto bytes = co_await std::visit(
            [&](auto& stream) {
               return client->read_body(connection, *stream, buffer, parser, output, policy, total_deadline,
                                        read_deadline, control);
            },
            connection->stream);
