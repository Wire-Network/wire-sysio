...
/** Throw a structured policy exception. */
[[noreturn]] void reject(ethereum_transaction_policy_reason reason,
...
fc::uint256 parse_canonical_uint256_decimal(std::string_view value,
   ...
   const auto invalid = [&] {
      reject(ethereum_transaction_policy_reason::configuration_value_invalid,
             field,
...
fc::uint256 parse_rpc_quantity(const fc::variant& value, std::string_view field) {
   if (!value.is_string()) {
      reject(ethereum_transaction_policy_reason::rpc_quantity_invalid, field, "<non-string>");
   }

   const std::string encoded = value.as_string();
   if (!encoded.starts_with(hex_quantity_prefix)) {
      reject(ethereum_transaction_policy_reason::rpc_quantity_invalid, field, "<invalid>");
   }
   ...
   if (digits.empty() || !hexadecimal_value || (digits.size() > 1 && digits.front() == '0')) {
      reject(ethereum_transaction_policy_reason::rpc_quantity_invalid,
             field,
             hexadecimal_value ? diagnostic_value(encoded) : "<invalid>");
   }
   if (digits.size() > max_uint256_hex_digits) {
      reject(ethereum_transaction_policy_reason::rpc_quantity_out_of_range, field, diagnostic_value(encoded));
   }
...
void validate_transaction_policy_configuration(const ethereum_transaction_policy& policy) {
   if (!is_safe_transaction_policy_identifier(policy.client_id)) {
      reject(ethereum_transaction_policy_reason::configuration_value_invalid, "client_id", "<invalid>");
   }
   if (policy.chain_id == 0) {
      reject(ethereum_transaction_policy_reason::configuration_value_invalid, "chain_id", "0");
   }
   ...
   if (policy.max_fee_per_gas == 0) {
      reject(ethereum_transaction_policy_reason::configuration_value_invalid, "max_fee_per_gas_wei", "0");
   }
   if (policy.max_gas_limit == 0) {
      reject(ethereum_transaction_policy_reason::configuration_value_invalid, "max_gas_limit", "0");
   }
   ...
   if (policy.max_priority_fee_per_gas > policy.max_fee_per_gas) {
      reject(ethereum_transaction_policy_reason::fee_relationship_invalid,
             "max_priority_fee_per_gas_wei",
...
fc::uint256 derive_max_fee_per_gas(const ethereum_transaction_policy& policy,
   ...
   if (priority_fee > policy.max_priority_fee_per_gas) {
      reject(ethereum_transaction_policy_reason::priority_fee_cap_exceeded,
   ...
   if (base_fee > maximum_base_fee) {
      reject(ethereum_transaction_policy_reason::max_fee_derivation_overflow,
             "max_fee_per_gas",
   ...
   if (max_fee > policy.max_fee_per_gas) {
      reject(ethereum_transaction_policy_reason::max_fee_cap_exceeded,
             "max_fee_per_gas",
             max_fee.str(),
             policy.max_fee_per_gas.str());
   }
...
fc::uint256 derive_buffered_gas_limit(const ethereum_transaction_policy& policy,
   ...
   if (estimated_gas > policy.max_gas_limit) {
      reject(ethereum_transaction_policy_reason::gas_limit_cap_exceeded,
             "estimated_gas",
             estimated_gas.str(),
   ...
   if (estimated_gas > maximum_estimate) {
      reject(ethereum_transaction_policy_reason::gas_limit_derivation_overflow,
             "gas_limit",
...
void validate_transaction_against_policy(const ethereum_transaction_policy&               policy,
   ...
   if (transaction.chain_id != policy.chain_id) {
      reject(ethereum_transaction_policy_reason::chain_id_mismatch,
             "chain_id",
   ...
   if (transaction.max_fee_per_gas > maximum_fee_without_overflow) {
      reject(ethereum_transaction_policy_reason::total_cost_multiplication_overflow,
   ...
   if (transaction.value > remaining_total_capacity) {
      reject(ethereum_transaction_policy_reason::total_cost_addition_overflow,
             "max_total_native_cost",
   ...
   if (maximum_total_cost > policy.max_total_native_cost) {
      reject(ethereum_transaction_policy_reason::total_cost_cap_exceeded,
             "max_total_native_cost",
