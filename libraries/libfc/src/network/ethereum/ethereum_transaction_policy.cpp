...
constexpr auto truncated_diagnostic_suffix = "...";
constexpr auto unavailable_allowed_value = "n/a";

namespace policy_field {
...
constexpr std::string_view max_fee_per_gas = "max_fee_per_gas";
constexpr std::string_view estimated_gas = "estimated_gas";
constexpr std::string_view gas_limit = "gas_limit";
constexpr std::string_view max_total_native_cost = "max_total_native_cost";
} // namespace policy_field

namespace diagnostic_observation {
constexpr auto invalid = "<invalid>";
constexpr auto non_string = "<non-string>";
constexpr auto zero = "0";
} // namespace diagnostic_observation

namespace diagnostic_operand {
constexpr auto base_fee = "base_fee=";
constexpr auto multiplier = ",multiplier=";
constexpr auto doubled_base_fee = "doubled_base_fee=";
constexpr auto priority_fee = ",priority_fee=";
constexpr auto derived_max_fee_per_gas = "derived_max_fee_per_gas=";
constexpr auto fee_formula = ",formula=2*base_fee_per_gas(";
constexpr auto priority_fee_formula = ")+max_priority_fee_per_gas(";
constexpr auto estimated_gas = "estimated_gas=";
constexpr auto gas_limit = "gas_limit=";
constexpr auto max_fee_per_gas = ",max_fee_per_gas=";
constexpr auto maximum_gas_cost = "maximum_gas_cost=";
...
std::string diagnostic_value(std::string_view value) {
   ...
   return std::string(value.substr(0, max_diagnostic_value_chars)) + truncated_diagnostic_suffix;
}
...
fc::uint256 parse_canonical_uint256_decimal(std::string_view value,
   ...
   const auto invalid = [&] {
      throw_transaction_policy_exception(
         ...
         field,
         numeric_value ? diagnostic_value(value) : diagnostic_observation::invalid);
   };
...
fc::uint256 parse_rpc_quantity(const fc::variant& value, std::string_view field) {
   if (!value.is_string()) {
      throw_transaction_policy_exception(
         ...
         field,
         diagnostic_observation::non_string);
   }
   ...
   if (digits.empty() || !hexadecimal_value || (digits.size() > 1 && digits.front() == '0')) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::rpc_quantity_invalid,
         field,
         hexadecimal_value ? diagnostic_value(encoded) : diagnostic_observation::invalid);
   }
...
void validate_transaction_policy_configuration(const ethereum_transaction_policy& policy) {
   if (!is_safe_transaction_policy_identifier(policy.client_id)) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::configuration_value_invalid,
         policy_field::client_id,
         diagnostic_observation::invalid);
   }
   if (policy.chain_id == 0) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::configuration_value_invalid,
         policy_field::chain_id,
         diagnostic_observation::zero);
   }
   ...
   if (policy.max_gas_limit == 0) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::configuration_value_invalid,
         policy_field::max_gas_limit,
         diagnostic_observation::zero);
   }
   if (policy.max_total_native_cost == 0) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::configuration_value_invalid,
         policy_field::max_total_native_cost_wei,
         diagnostic_observation::zero);
   }
...
fc::uint256 derive_max_fee_per_gas(const ethereum_transaction_policy& policy,
   ...
   if (priority_fee > policy.max_priority_fee_per_gas) {
      throw_transaction_policy_exception(ethereum_transaction_policy_reason::priority_fee_cap_exceeded,
                                         policy_field::max_priority_fee_per_gas,
                                         priority_fee.str(),
                                         policy.max_priority_fee_per_gas.str());
   }
   ...
   if (base_fee > maximum_base_fee) {
      throw_transaction_policy_exception(
         ...
         policy_field::max_fee_per_gas,
         diagnostic_operand::base_fee + base_fee.str() + diagnostic_operand::multiplier +
            std::to_string(max_fee_base_multiplier),
   ...
   if (priority_fee > remaining_fee_capacity) {
      throw_transaction_policy_exception(
         ethereum_transaction_policy_reason::max_fee_derivation_overflow,
         policy_field::max_fee_per_gas,
         diagnostic_operand::doubled_base_fee + doubled_base_fee.str() +
            diagnostic_operand::priority_fee + priority_fee.str(),
         maximum_ethereum_transaction_policy_value().str());
   }
   ...
   if (max_fee > policy.max_fee_per_gas) {
      throw_transaction_policy_exception(ethereum_transaction_policy_reason::max_fee_cap_exceeded,
                                         policy_field::max_fee_per_gas,
                                         diagnostic_operand::derived_max_fee_per_gas + max_fee.str() +
                                            diagnostic_operand::fee_formula + base_fee.str() +
                                            diagnostic_operand::priority_fee_formula + priority_fee.str() + ")",
                                         policy.max_fee_per_gas.str());
   }
   return max_fee;
}
...
fc::uint256 derive_buffered_gas_limit(const ethereum_transaction_policy& policy,
   ...
   if (estimated_gas > maximum_estimate) {
      throw_transaction_policy_exception(
         ...
         policy_field::gas_limit,
         diagnostic_operand::estimated_gas + estimated_gas.str() + diagnostic_operand::multiplier +
            std::to_string(gas_headroom_multiplier),
         maximum_ethereum_transaction_policy_value().str());
   }
...
void validate_transaction_against_policy(const ethereum_transaction_policy&               policy,
   ...
   if (transaction.max_fee_per_gas > maximum_fee_without_overflow) {
      throw_transaction_policy_exception(
         ...
         policy_field::max_total_native_cost,
         diagnostic_operand::gas_limit + transaction.gas_limit.str() +
            diagnostic_operand::max_fee_per_gas + transaction.max_fee_per_gas.str(),
         maximum_ethereum_transaction_policy_value().str());
   }
   ...
   if (transaction.value > remaining_total_capacity) {
      throw_transaction_policy_exception(
         ...
         policy_field::max_total_native_cost,
         diagnostic_operand::maximum_gas_cost + maximum_gas_cost.str() + diagnostic_operand::value +
