...
BOOST_AUTO_TEST_CASE(number_from_stream_negative_dot_integer_fit_double) {
   variant v = json::from_string("-.1");
   BOOST_CHECK(v.is_double());
   BOOST_CHECK_EQUAL(v.as_double(), -0.1);
}

BOOST_AUTO_TEST_CASE(number_from_stream_dot_integer_fit_double) {
   variant v = json::from_string(".1");
   BOOST_CHECK(v.is_double());
   BOOST_CHECK_EQUAL(v.as_double(), 0.1);
}
...
BOOST_AUTO_TEST_CASE(number_from_stream_negative_dot_throws) {
   BOOST_CHECK_THROW(json::from_string("-."), fc::parse_error_exception);
}

BOOST_AUTO_TEST_CASE(number_from_stream_dot_throws) {
   BOOST_CHECK_THROW(json::from_string("."), fc::parse_error_exception);
}
