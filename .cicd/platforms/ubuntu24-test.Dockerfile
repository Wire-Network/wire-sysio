...
# Environment variables matching main Dockerfile
...
# Set compiler to system clang-18 initially
ENV CC=/usr/bin/clang-18
ENV CXX=/usr/bin/clang++-18
ENV CMAKE_MAKE_PROGRAM=/usr/bin/ninja
ENV CMAKE_PREFIX_PATH="${LLVM_11_DIR}"

# Copy scripts with correct structure
COPY scripts /scripts
RUN ls -la /scripts

# Provide the path llvm-11 script expects (no copy; just a symlink)
