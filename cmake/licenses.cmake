...
# Collect license files for declared dependencies, trying common filenames used by vcpkg ports
...
# Non-package directories (cmake, doc, pkgconfig) and CMake compatibility aliases are skipped.
...
foreach(_dir IN LISTS _installed_dirs)
   if(IS_DIRECTORY "${_dir}")
      ...
      # Skip vcpkg-internal helpers and non-package directories
      if(_pkg MATCHES "^vcpkg-" OR _pkg STREQUAL "cmake" OR _pkg STREQUAL "doc" OR _pkg STREQUAL "pkgconfig")
         continue()
      endif()
      # Skip if already collected above
      if(_pkg IN_LIST _vcpkg_packages)
         continue()
      endif()
      # Silently collect license if present; transitive deps without a license
      # file are typically CMake compatibility aliases (e.g. nlohmann_json -> nlohmann-json)
      foreach(_license_name copyright LICENSE.md LICENSE LICENSE.txt)
         set(_src "${_vcpkg_share_dir}/${_pkg}/${_license_name}")
         if(EXISTS "${_src}")
            configure_file("${_src}" "licenses/sysio/LICENSE.${_pkg}" COPYONLY)
