...
`cleos` is distributed as part of the [Antelope software suite](https://github.com/AntelopeIO/wire_sysio). To install `cleos` just visit the [Antelope Software Installation](../00_install/index.md) section.

## Using Cleos

To use `cleos`, you need the end point (IP address and port number) of a running `nodeos` instance. Also, the `nodeos` instance must be configured to load the `sysio::chain_api_plugin` when launched. This allows `nodeos` to respond to the RPC requests coming from `cleos`.
