...
```sh
cleos set account permission test active '{"threshold":1,"keys":[{"key":"SYS8X7Mp7apQWtL6T2sfSZzBcQNUqZB7tARFEm9gA9Tn9nbMdsvBB","weight":1}],"accounts":[{"permission":{"actor":"acc2","permission":"active"},"weight":50}]}' owner
```
...
```sh
cleos set account permission test active '{"threshold" : 100, "keys" : [{"key":"SYS8X7Mp7apQWtL6T2sfSZzBcQNUqZB7tARFEm9gA9Tn9nbMdsvBB","weight":25}], "accounts" : [{"permission":{"actor":"sandwich","permission":"active"},"weight":75}]}' owner
```
...
```json
{
  ...
  "keys"            : [],     /*An array made up of individual permissions defined with an SYS PUBLIC KEY*/
  "accounts"        : []      /*An array made up of individual permissions defined with an SYS ACCOUNT*/
}
```
